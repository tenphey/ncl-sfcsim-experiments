#!/usr/bin/env python3
"""
E46 Experiment: Fixed B46 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e46/b46.properties,
which targets: 1.78 < NCCR_total <= 3.16 AND CCR_data < IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E46_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E46_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b46.properties")
...
DRY_RUN = int(os.getenv("E46_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E46_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E46_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E46_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e46_results.csv")
    results = []
    ...
    print("=== E46 Experiment: Fixed B46 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E46 - Fixed B46 Scenario (No Variable Sweep)",
    ...
    # Copy b46 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b46_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
