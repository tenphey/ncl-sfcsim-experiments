#!/usr/bin/env python3
"""
Analyze E46 results for the fixed B46 scenario (no variable sweep).
...
B46_MIN_REL_GAP_PCT = float(os.getenv("E46_B46_MIN_REL_GAP_PCT", "20.0"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e46_results.csv")
    ...
    df_all["b46_match"] = (
        ...
        & (df_all["CCR_IDR_rel_gap_pct"] >= B46_MIN_REL_GAP_PCT)
    )

    seed_metrics_csv = os.path.join(result_dir, "e46_seed_metrics_with_b46_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B46 condition
    df = df_algo_valid[df_algo_valid["b46_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B46 condition: 1.78 < NCCR_total <= 3.16 AND "
            f"CCR_data < IDR_image AND relative CCR/IDR gap >= {B46_MIN_REL_GAP_PCT:.2f}%"
        )
    ...
    summary = {
        "scenario": "b46",
        "b46_min_rel_gap_pct": B46_MIN_REL_GAP_PCT,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b46_qualified_runs": qualified_runs,
        "b46_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e46_summary.csv")
    ...
    loss_json = {"b46": seed_map}
    loss_json_path = os.path.join(result_dir, "e46_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B46 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E46 Results (B46 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b46_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e46_summary_table.png")
    draw_table_image(table_df, "E46 Summary (B46 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e46_robust_table.png")
    draw_table_image(robust_print, "E46 Robust Statistics (B46 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E46 Mean Makespan Comparison (B46 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e46_makespan_mean_comparison.png")
    ...
    ax.set_title("E46 Makespan Distribution by Seed (B46 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e46_makespan_boxplot.png")
    ...
    ax.set_title("E46 Gain Distribution (B46 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e46_gain_histogram.png")
