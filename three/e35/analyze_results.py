#!/usr/bin/env python3
"""
Analyze E35 results for the fixed B35 scenario (no variable sweep).
...
B35_REL_TOL_PCT = float(os.getenv("E35_B35_REL_TOL_PCT", "10.0"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e35_results.csv")
    ...
    df_all["b35_match"] = (
        ...
        & (df_all["CCR_IDR_rel_gap_pct"] <= B35_REL_TOL_PCT)
    )

    seed_metrics_csv = os.path.join(result_dir, "e35_seed_metrics_with_b35_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B35 condition
    df = df_algo_valid[df_algo_valid["b35_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B35 condition: 1.00 < NCCR_total <= 1.78 AND "
            f"relative CCR/IDR gap <= {B35_REL_TOL_PCT:.2f}%"
        )
    ...
    summary = {
        "scenario": "b35",
        "b35_rel_tolerance_pct": B35_REL_TOL_PCT,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b35_qualified_runs": qualified_runs,
        "b35_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e35_summary.csv")
    ...
    loss_json = {"b35": seed_map}
    loss_json_path = os.path.join(result_dir, "e35_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B35 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E35 Results (B35 Fixed Scenario) ===")
    wins_n_over_d_display = f"{wins_n_over_d}/{qualified_runs} ({summary['win_rate_N_over_D'] * 100:.2f}%)"
    console_row = {
        "scenario": "b35",
        "b35_rel_tol_pct": B35_REL_TOL_PCT,
        "algo_valid_runs": f"{algo_valid_runs}/{total_runs}",
        "b35_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e35_summary_table.png")
    draw_table_image(table_df, "E35 Summary (B35 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e35_robust_table.png")
    draw_table_image(robust_print, "E35 Robust Statistics (B35 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E35 Mean Makespan Comparison (B35 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e35_makespan_mean_comparison.png")
    ...
    ax.set_title("E35 Makespan Distribution by Seed (B35 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e35_makespan_boxplot.png")
    ...
    ax.set_title("E35 Gain Distribution (B35 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e35_gain_histogram.png")
