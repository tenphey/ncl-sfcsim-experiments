#!/usr/bin/env python3
"""
E35 Experiment: Fixed B35 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e35/b35.properties,
which targets: 1.00 < NCCR_total <= 1.78 AND CCR_data ~= IDR_image.

Usage:
  python3 run_experiment.py                     # Normal run
  E35_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E35_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b35.properties")
...
DRY_RUN = int(os.getenv("E35_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E35_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E35_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E35_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e35_results.csv")
    results = []
    ...
    print("=== E35 Experiment: Fixed B35 Scenario (No Variable Sweep) ===")
    print("Target scenario: 1.00 < NCCR_total <= 1.78 AND CCR_data ~= IDR_image")
    ...
    # Write manifest
    manifest = {
        "experiment": "E35 - Fixed B35 Scenario (No Variable Sweep)",
        "scenario": "1.00 < NCCR_total <= 1.78 AND CCR_data ~= IDR_image",
        "timestamp": timestamp,
    ...
    # Copy b35 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b35_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
