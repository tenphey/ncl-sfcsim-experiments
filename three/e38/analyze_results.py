#!/usr/bin/env python3
"""
Analyze E38 results for the fixed B38 scenario (no variable sweep).
...
B38_REL_TOL_PCT = float(os.getenv("E38_B38_REL_TOL_PCT", "10.0"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e38_results.csv")
    ...
    df_all["b38_match"] = (
        ...
        & (df_all["CCR_IDR_rel_gap_pct"] <= B38_REL_TOL_PCT)
    )

    seed_metrics_csv = os.path.join(result_dir, "e38_seed_metrics_with_b38_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B38 condition
    df = df_algo_valid[df_algo_valid["b38_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B38 condition: 5.62 < NCCR_total <= 10.00 AND "
            f"relative CCR/IDR gap <= {B38_REL_TOL_PCT:.2f}%"
        )
    ...
    summary = {
        "scenario": "b38",
        "b38_rel_tolerance_pct": B38_REL_TOL_PCT,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b38_qualified_runs": qualified_runs,
        "b38_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e38_summary.csv")
    ...
    loss_json = {"b38": seed_map}
    loss_json_path = os.path.join(result_dir, "e38_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B38 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E38 Results (B38 Fixed Scenario) ===")
    wins_n_over_d_display = f"{wins_n_over_d}/{qualified_runs} ({summary['win_rate_N_over_D'] * 100:.2f}%)"
    console_row = {
        "scenario": "b38",
        "b38_rel_tol_pct": B38_REL_TOL_PCT,
        "algo_valid_runs": f"{algo_valid_runs}/{total_runs}",
        "b38_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e38_summary_table.png")
    draw_table_image(table_df, "E38 Summary (B38 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e38_robust_table.png")
    draw_table_image(robust_print, "E38 Robust Statistics (B38 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E38 Mean Makespan Comparison (B38 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e38_makespan_mean_comparison.png")
    ...
    ax.set_title("E38 Makespan Distribution by Seed (B38 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e38_makespan_boxplot.png")
    ...
    ax.set_title("E38 Gain Distribution (B38 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e38_gain_histogram.png")
