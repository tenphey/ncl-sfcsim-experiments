#!/usr/bin/env python3
"""
E57 Experiment: Fixed B57 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e57/b57.properties,
which targets: 3.16 < NCCR_total <= 5.62 AND CCR_data > IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E57_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E57_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b57.properties")

DRY_RUN = int(os.getenv("E57_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E57_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E57_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E57_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e57_results.csv")
    results = []
    ...
    print("=== E57 Experiment: Fixed B57 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E57 - Fixed B57 Scenario (No Variable Sweep)",
    ...
    # Copy b57 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b57_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
