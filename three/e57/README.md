# E57: Fixed B57 Scenario in the Seventh NCCR Bin

## Purpose

E57 is the seventh experiment in the new 8-bin `NCCR_total` study.
...
The role of E57 is to observe the behavior of `HEFT`, `DHEFT`, and `NHEFT`
...
E57 is **not** a parameter sweep experiment.

It is a **fixed-scenario repeated-seed experiment**:

- one tuned property set: `b57.properties`
- many random seeds
- final statistics only on seeds that actually satisfy the B57 condition
...
E57 corresponds to the seventh bucket:
...
E57 is the data-dominant counterpart of the balanced `E3X` series.
...
`b57.properties` is derived from the corresponding `E3X` profile, but shifted
so that:
...
With the current `b57.properties` and `random_seed=2503`, the project produced:
...
```bash
python3 experiments/e57/run_experiment.py
```

### Dry run

```bash
E57_DRY_RUN=1 python3 experiments/e57/run_experiment.py
```

### Fewer seeds

```bash
E57_NUM_SEEDS=10 python3 experiments/e57/run_experiment.py
```

### Analyze one run

```bash
python3 experiments/e57/analyze_results.py run_YYYYMMDD_HHMMSS_150_100
```

## Important Counting Rule

E57 follows the same counting logic as the earlier fixed-scenario experiments:

1. `run_experiment.py` executes all selected seeds and stores all raw results.
2. `analyze_results.py` re-parses each run log.
3. Only runs that actually satisfy the B57 condition are counted in the final
   summary.
...
- `grid_e57_results.csv`
- `run_manifest.json`
- `b57_properties_snapshot.properties`
- `logs/run_seed_<seed>.log`

The analysis script produces:

- `grid_e57_summary.csv`
- `e57_seed_metrics_with_b57_flag.csv`
- `e57_nheft_loss_seeds_by_scenario.json`
- `e57_summary_table.png`
- `e57_robust_table.png`
- `e57_makespan_mean_comparison.png`
- `e57_makespan_boxplot.png`
- `e57_gain_histogram.png`
