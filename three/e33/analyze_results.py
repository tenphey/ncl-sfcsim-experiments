#!/usr/bin/env python3
"""
Analyze E33 results for the fixed B33 scenario (no variable sweep).
...
B33_ABS_TOL = float(os.getenv("E33_B33_ABS_TOL", "0.02"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e33_results.csv")
    ...
    df_all["b33_match"] = (
        ...
        & (df_all["CCR_IDR_abs_diff"] <= B33_ABS_TOL)
    )

    seed_metrics_csv = os.path.join(result_dir, "e33_seed_metrics_with_b33_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B33 condition
    df = df_algo_valid[df_algo_valid["b33_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B33 condition: 0.32 < NCCR_total <= 0.56 AND "
            f"|CCR_data-IDR_image| <= {B33_ABS_TOL:.4f}"
        )
    ...
    summary = {
        "scenario": "b33",
        "b33_abs_tolerance": B33_ABS_TOL,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b33_qualified_runs": qualified_runs,
        "b33_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e33_summary.csv")
    ...
    loss_json = {"b33": seed_map}
    loss_json_path = os.path.join(result_dir, "e33_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B33 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E33 Results (B33 Fixed Scenario) ===")
    wins_n_over_d_display = f"{wins_n_over_d}/{qualified_runs} ({summary['win_rate_N_over_D'] * 100:.2f}%)"
    console_row = {
        "scenario": "b33",
        "b33_abs_tol": B33_ABS_TOL,
        "algo_valid_runs": f"{algo_valid_runs}/{total_runs}",
        "b33_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e33_summary_table.png")
    draw_table_image(table_df, "E33 Summary (B33 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e33_robust_table.png")
    draw_table_image(robust_print, "E33 Robust Statistics (B33 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E33 Mean Makespan Comparison (B33 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e33_makespan_mean_comparison.png")
    ...
    ax.set_title("E33 Makespan Distribution by Seed (B33 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e33_makespan_boxplot.png")
    ...
    ax.set_title("E33 Gain Distribution (B33 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e33_gain_histogram.png")
