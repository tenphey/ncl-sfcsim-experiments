#!/usr/bin/env python3
"""
Analyze E43 results for the fixed B43 scenario (no variable sweep).
...
B43_MIN_REL_GAP_PCT = float(os.getenv("E43_B43_MIN_REL_GAP_PCT", "20.0"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e43_results.csv")
    ...
    df_all["b43_match"] = (
        ...
        & (df_all["CCR_IDR_rel_gap_pct"] >= B43_MIN_REL_GAP_PCT)
    )

    seed_metrics_csv = os.path.join(result_dir, "e43_seed_metrics_with_b43_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B43 condition
    df = df_algo_valid[df_algo_valid["b43_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B43 condition: 0.32 < NCCR_total <= 0.56 AND "
            f"CCR_data < IDR_image AND relative CCR/IDR gap >= {B43_MIN_REL_GAP_PCT:.2f}%"
        )
    ...
    summary = {
        "scenario": "b43",
        "b43_min_rel_gap_pct": B43_MIN_REL_GAP_PCT,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b43_qualified_runs": qualified_runs,
        "b43_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e43_summary.csv")
    ...
    loss_json = {"b43": seed_map}
    loss_json_path = os.path.join(result_dir, "e43_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B43 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E43 Results (B43 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b43_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e43_summary_table.png")
    draw_table_image(table_df, "E43 Summary (B43 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e43_robust_table.png")
    draw_table_image(robust_print, "E43 Robust Statistics (B43 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E43 Mean Makespan Comparison (B43 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e43_makespan_mean_comparison.png")
    ...
    ax.set_title("E43 Makespan Distribution by Seed (B43 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e43_makespan_boxplot.png")
    ...
    ax.set_title("E43 Gain Distribution (B43 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e43_gain_histogram.png")
