#!/usr/bin/env python3
"""
E41 Experiment: Fixed B41 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e41/b41.properties,
which targets: 0.10 < NCCR_total <= 0.18 AND CCR_data < IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E41_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E41_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b41.properties")

DRY_RUN = int(os.getenv("E41_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E41_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E41_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E41_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e41_results.csv")
    results = []
    ...
    print("=== E41 Experiment: Fixed B41 Scenario (No Variable Sweep) ===")
    print("Target scenario: 0.10 < NCCR_total <= 0.18 AND CCR_data < IDR_image (relative gap >= 20%)")
    ...
    # Write manifest
    manifest = {
        "experiment": "E41 - Fixed B41 Scenario (No Variable Sweep)",
        "scenario": "0.10 < NCCR_total <= 0.18 AND CCR_data < IDR_image (relative gap >= 20%)",
        "timestamp": timestamp,
    ...
    # Copy b41 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b41_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
