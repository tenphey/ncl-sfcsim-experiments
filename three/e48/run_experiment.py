#!/usr/bin/env python3
"""
E48 Experiment: Fixed B48 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e48/b48.properties,
which targets: 5.62 < NCCR_total <= 10.00 AND CCR_data < IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E48_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E48_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b48.properties")
...
DRY_RUN = int(os.getenv("E48_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E48_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E48_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E48_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e48_results.csv")
    results = []
    ...
    print("=== E48 Experiment: Fixed B48 Scenario (No Variable Sweep) ===")
    print("Target scenario: 5.62 < NCCR_total <= 10.00 AND CCR_data < IDR_image (relative gap >= 20%)")
    ...
    # Write manifest
    manifest = {
        "experiment": "E48 - Fixed B48 Scenario (No Variable Sweep)",
        "scenario": "5.62 < NCCR_total <= 10.00 AND CCR_data < IDR_image (relative gap >= 20%)",
        "timestamp": timestamp,
    ...
    # Copy b48 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b48_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
