#!/usr/bin/env python3
"""
E56 Experiment: Fixed B56 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e56/b56.properties,
which targets: 1.78 < NCCR_total <= 3.16 AND CCR_data > IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E56_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E56_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b56.properties")
...
DRY_RUN = int(os.getenv("E56_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E56_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E56_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E56_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e56_results.csv")
    results = []
    ...
    print("=== E56 Experiment: Fixed B56 Scenario (No Variable Sweep) ===")
    print("Target scenario: 1.78 < NCCR_total <= 3.16 AND CCR_data > IDR_image (relative gap >= 20%)")
    ...
    # Write manifest
    manifest = {
        "experiment": "E56 - Fixed B56 Scenario (No Variable Sweep)",
        "scenario": "1.78 < NCCR_total <= 3.16 AND CCR_data > IDR_image (relative gap >= 20%)",
        "timestamp": timestamp,
    ...
    # Copy b56 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b56_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
