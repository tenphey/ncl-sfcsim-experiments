#!/usr/bin/env python3
"""
Analyze E34 results for the fixed B34 scenario (no variable sweep).
...
B34_REL_TOL_PCT = float(os.getenv("E34_B34_REL_TOL_PCT", "10.0"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e34_results.csv")
    ...
    df_all["b34_match"] = (
        ...
        & (df_all["CCR_IDR_rel_gap_pct"] <= B34_REL_TOL_PCT)
    )

    seed_metrics_csv = os.path.join(result_dir, "e34_seed_metrics_with_b34_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B34 condition
    df = df_algo_valid[df_algo_valid["b34_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B34 condition: 0.56 < NCCR_total <= 1.00 AND "
            f"relative CCR/IDR gap <= {B34_REL_TOL_PCT:.2f}%"
        )
    ...
    summary = {
        "scenario": "b34",
        "b34_rel_tolerance_pct": B34_REL_TOL_PCT,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b34_qualified_runs": qualified_runs,
        "b34_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e34_summary.csv")
    ...
    loss_json = {"b34": seed_map}
    loss_json_path = os.path.join(result_dir, "e34_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B34 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E34 Results (B34 Fixed Scenario) ===")
    wins_n_over_d_display = f"{wins_n_over_d}/{qualified_runs} ({summary['win_rate_N_over_D'] * 100:.2f}%)"
    console_row = {
        "scenario": "b34",
        "b34_rel_tol_pct": B34_REL_TOL_PCT,
        "algo_valid_runs": f"{algo_valid_runs}/{total_runs}",
        "b34_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e34_summary_table.png")
    draw_table_image(table_df, "E34 Summary (B34 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e34_robust_table.png")
    draw_table_image(robust_print, "E34 Robust Statistics (B34 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E34 Mean Makespan Comparison (B34 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e34_makespan_mean_comparison.png")
    ...
    ax.set_title("E34 Makespan Distribution by Seed (B34 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e34_makespan_boxplot.png")
    ...
    ax.set_title("E34 Gain Distribution (B34 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e34_gain_histogram.png")
