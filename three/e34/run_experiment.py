#!/usr/bin/env python3
"""
E34 Experiment: Fixed B34 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e34/b34.properties,
which targets: 0.56 < NCCR_total <= 1.00 AND CCR_data ~= IDR_image.

Usage:
  python3 run_experiment.py                     # Normal run
  E34_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E34_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
# Seeding
MASTER_SEED = int(os.getenv("E34_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E34_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e34_results.csv")
    results = []
    ...
    print("=== E34 Experiment: Fixed B34 Scenario (No Variable Sweep) ===")
    print("Target scenario: 0.56 < NCCR_total <= 1.00 AND CCR_data ~= IDR_image")
    ...
    # Write manifest
    manifest = {
        "experiment": "E34 - Fixed B34 Scenario (No Variable Sweep)",
        "scenario": "0.56 < NCCR_total <= 1.00 AND CCR_data ~= IDR_image",
        "timestamp": timestamp,
