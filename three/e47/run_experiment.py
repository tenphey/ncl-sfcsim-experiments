#!/usr/bin/env python3
"""
E47 Experiment: Fixed B47 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e47/b47.properties,
which targets: 3.16 < NCCR_total <= 5.62 AND CCR_data < IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E47_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E47_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b47.properties")

DRY_RUN = int(os.getenv("E47_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E47_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E47_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E47_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e47_results.csv")
    results = []
    ...
    print("=== E47 Experiment: Fixed B47 Scenario (No Variable Sweep) ===")
    print("Target scenario: 3.16 < NCCR_total <= 5.62 AND CCR_data < IDR_image (relative gap >= 20%)")
    ...
    # Write manifest
    manifest = {
        "experiment": "E47 - Fixed B47 Scenario (No Variable Sweep)",
        "scenario": "3.16 < NCCR_total <= 5.62 AND CCR_data < IDR_image (relative gap >= 20%)",
        "timestamp": timestamp,
    ...
    # Copy b47 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b47_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
