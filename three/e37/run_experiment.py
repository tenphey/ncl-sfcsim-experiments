#!/usr/bin/env python3
"""
E37 Experiment: Fixed B37 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e37/b37.properties,
which targets: 3.16 < NCCR_total <= 5.62 AND CCR_data ~= IDR_image.

Usage:
  python3 run_experiment.py                     # Normal run
  E37_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E37_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b37.properties")
...
DRY_RUN = int(os.getenv("E37_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E37_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E37_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E37_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e37_results.csv")
    results = []
    ...
    print("=== E37 Experiment: Fixed B37 Scenario (No Variable Sweep) ===")
    print("Target scenario: 3.16 < NCCR_total <= 5.62 AND CCR_data ~= IDR_image")
    ...
    # Write manifest
    manifest = {
        "experiment": "E37 - Fixed B37 Scenario (No Variable Sweep)",
        "scenario": "3.16 < NCCR_total <= 5.62 AND CCR_data ~= IDR_image",
        "timestamp": timestamp,
    ...
    # Copy b37 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b37_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
