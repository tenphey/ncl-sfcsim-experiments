#!/usr/bin/env python3
"""
Aggregate latest runs of e51-e58 and draw one grouped makespan bar chart.

Output folder:
  experiments/e51_e58/<timestamp>/
...
OUT_BASE_DIR = os.path.join(THIS_DIR, "e51_e58")
PROJECT_ROOT = os.path.dirname(THIS_DIR)

SCENARIOS = [
    {"exp_dir": "e51", "label": "(0.10, 0.18]", "summary_csv": "grid_e51_summary.csv", "scenario": "b51", "constraint": "CCR > IDR, rel gap >= 20%"},
    {"exp_dir": "e52", "label": "(0.18, 0.32]", "summary_csv": "grid_e52_summary.csv", "scenario": "b52", "constraint": "CCR > IDR, rel gap >= 20%"},
    {"exp_dir": "e53", "label": "(0.32, 0.56]", "summary_csv": "grid_e53_summary.csv", "scenario": "b53", "constraint": "CCR > IDR, rel gap >= 20%"},
    {"exp_dir": "e54", "label": "(0.56, 1.00]", "summary_csv": "grid_e54_summary.csv", "scenario": "b54", "constraint": "CCR > IDR, rel gap >= 20%"},
    {"exp_dir": "e55", "label": "(1.00, 1.78]", "summary_csv": "grid_e55_summary.csv", "scenario": "b55", "constraint": "CCR > IDR, rel gap >= 20%"},
    {"exp_dir": "e56", "label": "(1.78, 3.16]", "summary_csv": "grid_e56_summary.csv", "scenario": "b56", "constraint": "CCR > IDR, rel gap >= 20%"},
    {"exp_dir": "e57", "label": "(3.16, 5.62]", "summary_csv": "grid_e57_summary.csv", "scenario": "b57", "constraint": "CCR > IDR, rel gap >= 20%"},
    {"exp_dir": "e58", "label": "(5.62, 10.00]", "summary_csv": "grid_e58_summary.csv", "scenario": "b58", "constraint": "CCR > IDR, rel gap >= 20%"},
]
...
def draw_grouped_bar_chart(df, out_path):
    ...
    ax.set_title("E51-E58 Makespan Mean Comparison", fontsize=TITLE_FONTSIZE, fontweight="bold", pad=16)
    ax.set_xticks(x)
    ...
    fig.text(
        0.5,
        0.02,
        "Target family: CCR_data > IDR_image   |   all e51-e58 bins use relative CCR/IDR gap >= 20%",
        ha="center",
...
def draw_vcpu_usage_chart(df, out_path):
    ...
    ax.set_xlabel("NCCR Bucket", fontsize=AXIS_LABEL_FONTSIZE)
    ax.set_title("E51-E58 vCPU Usage Comparison", fontsize=TITLE_FONTSIZE, fontweight="bold", pad=16)
    ax.set_xticks(x)
    ax.set_xticklabels(df["bucket_label"].tolist(), rotation=0, fontsize=XTICK_FONTSIZE)
    ax.tick_params(axis="y", labelsize=YTICK_FONTSIZE)
    ax.grid(axis="y", linestyle="--", alpha=0.35)
...
def build_summary_text(df):
    lines = []
    lines.append("E51-E58 Aggregated Summary")
    lines.append("")
    ...
    lines.append("- e51-e58 all require CCR_data > IDR_image")
    lines.append("- e51-e58 all require relative CCR/IDR gap >= 20%")
    return "\n".join(lines) + "\n"
...
def main():
    ...
    summary_csv_path = os.path.join(out_dir, "e5x_latest_summary.csv")
