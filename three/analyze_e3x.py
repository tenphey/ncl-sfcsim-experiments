#!/usr/bin/env python3
"""
Aggregate latest runs of e31-e38 and draw one grouped makespan bar chart.

Output folder:
  experiments/e31_e38/<timestamp>/
...
OUT_BASE_DIR = os.path.join(THIS_DIR, "e31_e38")
...
SCENARIOS = [
    {
        "exp_dir": "e31",
        "label": "(0.10, 0.18]",
        "summary_csv": "grid_e31_summary.csv",
        "scenario": "b31",
        "constraint": "abs(CCR-IDR) <= 0.02",
    },
    {
        "exp_dir": "e32",
        "label": "(0.18, 0.32]",
        "summary_csv": "grid_e32_summary.csv",
        "scenario": "b32",
        "constraint": "abs(CCR-IDR) <= 0.02",
    },
    {
        "exp_dir": "e33",
        "label": "(0.32, 0.56]",
        "summary_csv": "grid_e33_summary.csv",
        "scenario": "b33",
        "constraint": "abs(CCR-IDR) <= 0.02",
    },
    {
        "exp_dir": "e34",
        "label": "(0.56, 1.00]",
        "summary_csv": "grid_e34_summary.csv",
        "scenario": "b34",
        "constraint": "rel(CCR,IDR) <= 10%",
    },
    {
        "exp_dir": "e35",
        "label": "(1.00, 1.78]",
        "summary_csv": "grid_e35_summary.csv",
        "scenario": "b35",
        "constraint": "rel(CCR,IDR) <= 10%",
    },
    {
        "exp_dir": "e36",
        "label": "(1.78, 3.16]",
        "summary_csv": "grid_e36_summary.csv",
        "scenario": "b36",
        "constraint": "rel(CCR,IDR) <= 10%",
    },
    {
        "exp_dir": "e37",
        "label": "(3.16, 5.62]",
        "summary_csv": "grid_e37_summary.csv",
        "scenario": "b37",
        "constraint": "rel(CCR,IDR) <= 10%",
    },
    {
        "exp_dir": "e38",
        "label": "(5.62, 10.00]",
        "summary_csv": "grid_e38_summary.csv",
        "scenario": "b38",
        "constraint": "rel(CCR,IDR) <= 10%",
    },
]
...
def draw_grouped_bar_chart(df, out_path):
    ...
    ax.set_title("E31-E38 Makespan Mean Comparison", fontsize=TITLE_FONTSIZE, fontweight="bold", pad=16)
    ax.set_xticks(x)
    ...
    # Right axis 1: NHEFT win rate over DHEFT
    ax2 = ax.twinx()
    win_line, = ax2.plot(
        x,
        win_rate_vals,
    ...
    fig.text(
        0.5,
        0.02,
        "Target family: CCR_data ~= IDR_image   |   e31-e33: absolute tolerance <= 0.02   |   e34-e38: relative tolerance <= 10%",
        ha="center",
...
def draw_vcpu_usage_chart(df, out_path):
    ...
    ax.set_xlabel("NCCR Bucket", fontsize=AXIS_LABEL_FONTSIZE)
    ax.set_title("E31-E38 vCPU Usage Comparison", fontsize=TITLE_FONTSIZE, fontweight="bold", pad=16)
    ax.set_xticks(x)
    ax.set_xticklabels(df["bucket_label"].tolist(), rotation=0, fontsize=XTICK_FONTSIZE)
    ax.tick_params(axis="y", labelsize=YTICK_FONTSIZE)
    ax.grid(axis="y", linestyle="--", alpha=0.35)
...
def build_summary_text(df):
    lines = []
    lines.append("E31-E38 Aggregated Summary")
    lines.append("")
    ...
    lines.append("- e31-e33 use absolute CCR/IDR tolerance <= 0.02")
    lines.append("- e34-e38 use relative CCR/IDR tolerance <= 10%")
    return "\n".join(lines) + "\n"
...
def main():
    ...
    summary_csv_path = os.path.join(out_dir, "e3x_latest_summary.csv")
