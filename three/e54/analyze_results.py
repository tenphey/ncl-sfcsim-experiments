#!/usr/bin/env python3
"""
Analyze E54 results for the fixed B54 scenario (no variable sweep).
...
B54_MIN_REL_GAP_PCT = float(os.getenv("E54_B54_MIN_REL_GAP_PCT", "20.0"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e54_results.csv")
    ...
    df_all["b54_match"] = (
        (df_all["NCCR_total"] > 0.56)
        & (df_all["NCCR_total"] <= 1.00)
        & (df_all["CCR_data"] > df_all["IDR_image"])
        & (df_all["CCR_IDR_rel_gap_pct"] >= B54_MIN_REL_GAP_PCT)
    )

    seed_metrics_csv = os.path.join(result_dir, "e54_seed_metrics_with_b54_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B54 condition
    df = df_algo_valid[df_algo_valid["b54_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B54 condition: 0.56 < NCCR_total <= 1.00 AND "
            f"CCR_data > IDR_image AND relative CCR/IDR gap >= {B54_MIN_REL_GAP_PCT:.2f}%"
        )
    ...
    summary = {
        "scenario": "b54",
        "b54_min_rel_gap_pct": B54_MIN_REL_GAP_PCT,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b54_qualified_runs": qualified_runs,
        "b54_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e54_summary.csv")
    ...
    loss_json = {"b54": seed_map}
    loss_json_path = os.path.join(result_dir, "e54_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B54 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E54 Results (B54 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b54_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e54_summary_table.png")
    draw_table_image(table_df, "E54 Summary (B54 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e54_robust_table.png")
    draw_table_image(robust_print, "E54 Robust Statistics (B54 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E54 Mean Makespan Comparison (B54 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e54_makespan_mean_comparison.png")
    ...
    ax.set_title("E54 Makespan Distribution by Seed (B54 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e54_makespan_boxplot.png")
    ...
    ax.set_title("E54 Gain Distribution (B54 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e54_gain_histogram.png")
