#!/usr/bin/env python3
"""
E55 Experiment: Fixed B55 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e55/b55.properties,
which targets: 1.00 < NCCR_total <= 1.78 AND CCR_data > IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E55_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E55_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b55.properties")

DRY_RUN = int(os.getenv("E55_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E55_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E55_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E55_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e55_results.csv")
    results = []
    ...
    print("=== E55 Experiment: Fixed B55 Scenario (No Variable Sweep) ===")
    print("Target scenario: 1.00 < NCCR_total <= 1.78 AND CCR_data > IDR_image (relative gap >= 20%)")
    ...
    # Write manifest
    manifest = {
        "experiment": "E55 - Fixed B55 Scenario (No Variable Sweep)",
        "scenario": "1.00 < NCCR_total <= 1.78 AND CCR_data > IDR_image (relative gap >= 20%)",
        "timestamp": timestamp,
    ...
    # Copy b55 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b55_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
