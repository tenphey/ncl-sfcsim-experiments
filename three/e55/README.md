# E55: Fixed B55 Scenario in the Fifth NCCR Bin

## Purpose

E55 is the fifth experiment in the new 8-bin `NCCR_total` study.
...
The role of E55 is to observe the behavior of `HEFT`, `DHEFT`, and `NHEFT`
...
E55 is **not** a parameter sweep experiment.

It is a **fixed-scenario repeated-seed experiment**:

- one tuned property set: `b55.properties`
- many random seeds
- final statistics only on seeds that actually satisfy the B55 condition
...
E55 corresponds to the fifth bucket:
...
E55 is the data-dominant counterpart of the balanced `E3X` series.
...
`b55.properties` is derived from the corresponding `E3X` profile, but shifted
so that:
...
With the current `b55.properties` and `random_seed=2503`, the project produced:
...
```bash
python3 experiments/e55/run_experiment.py
```

### Dry run

```bash
E55_DRY_RUN=1 python3 experiments/e55/run_experiment.py
```

### Fewer seeds

```bash
E55_NUM_SEEDS=10 python3 experiments/e55/run_experiment.py
```

### Analyze one run

```bash
python3 experiments/e55/analyze_results.py run_YYYYMMDD_HHMMSS_150_100
```

## Important Counting Rule

E55 follows the same counting logic as the earlier fixed-scenario experiments:

1. `run_experiment.py` executes all selected seeds and stores all raw results.
2. `analyze_results.py` re-parses each run log.
3. Only runs that actually satisfy the B55 condition are counted in the final
   summary.
...
- `grid_e55_results.csv`
- `run_manifest.json`
- `b55_properties_snapshot.properties`
- `logs/run_seed_<seed>.log`

The analysis script produces:

- `grid_e55_summary.csv`
- `e55_seed_metrics_with_b55_flag.csv`
- `e55_nheft_loss_seeds_by_scenario.json`
- `e55_summary_table.png`
- `e55_robust_table.png`
- `e55_makespan_mean_comparison.png`
- `e55_makespan_boxplot.png`
- `e55_gain_histogram.png`
