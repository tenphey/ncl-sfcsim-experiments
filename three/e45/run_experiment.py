#!/usr/bin/env python3
"""
E45 Experiment: Fixed B45 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e45/b45.properties,
which targets: 1.00 < NCCR_total <= 1.78 AND CCR_data < IDR_image (relative gap >= 20%).

Usage:
  python3 run_experiment.py                     # Normal run
  E45_DRY_RUN=1 python3 run_experiment.py       # Dry run (print commands only)
  E45_NUM_SEEDS=10 python3 run_experiment.py    # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b45.properties")
...
DRY_RUN = int(os.getenv("E45_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E45_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E45_MASTER_SEED", "151"))
NUM_SEEDS_DEFAULT = 500
NUM_SEEDS = int(os.getenv("E45_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e45_results.csv")
    results = []
    ...
    print("=== E45 Experiment: Fixed B45 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E45 - Fixed B45 Scenario (No Variable Sweep)",
    ...
    # Copy b45 properties snapshot
    if os.path.exists(BASE_PROPS):
        with open(BASE_PROPS) as src:
            content = src.read()
        with open(os.path.join(out_dir, "b45_properties_snapshot.properties"), "w") as dst:
            dst.write(content)
