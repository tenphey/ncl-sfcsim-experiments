#!/usr/bin/env python3
"""
Analyze E21 results for the fixed B1 scenario (no variable sweep).
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e21_results.csv")
    ...
    seed_metrics_csv = os.path.join(result_dir, "e21_seed_metrics_with_b1_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B1 condition
    df = df_algo_valid[df_algo_valid["b1_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print("No runs satisfy B1 condition: 0 < NCCR_total < 1 AND CCR_data > IDR_image")
    ...
    summary = {
        ...
        "b1_qualified_runs": qualified_runs,
        "b1_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e21_summary.csv")
    ...
    loss_json = {"b1": seed_map}
    loss_json_path = os.path.join(result_dir, "e21_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B1 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E21 Results (B1 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b1_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e21_summary_table.png")
    draw_table_image(table_df, "E21 Summary (B1 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e21_robust_table.png")
    draw_table_image(robust_print, "E21 Robust Statistics (B1 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E21 Mean Makespan Comparison (B1 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e21_makespan_mean_comparison.png")
    ...
    ax.set_title("E21 Makespan Distribution by Seed (B1 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e21_makespan_boxplot.png")
    ...
    ax.set_title("E21 Gain Distribution (B1 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e21_gain_histogram.png")
