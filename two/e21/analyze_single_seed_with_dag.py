#!/usr/bin/env python3
"""
Single Seed Deep Dive Analysis for E21 (B1 Fixed Scenario)

【用途】分析单个 seed 的调度详情（HEFT / DHEFT / NHEFT 对比）

【使用方法】
  python3 analyze_single_seed2.py <seed> [--dag|--no-dag]
...
【使用例子】
  python3 analyze_single_seed2.py 7476
  python3 analyze_single_seed2.py 7476 --no-dag

【输出说明】
  - 三个算法的 makespan 对比 (HEFT vs DHEFT vs NHEFT)
  - CCR_data / IDR_image / NCCR_total
  - 该 seed 是否满足 b1 条件：0 < NCCR_total < 1 AND CCR_data > IDR_image
...
def parse_dag_flag(argv):
    """
    Default: DAG export ON for analyze_single_seed2.
    Supports:
      --dag / dag / on / true / 1
      --no-dag / off / false / 0
    """
...
def analyze():
    if len(sys.argv) < 2:
        print('Usage: python3 analyze_single_seed2.py <seed> [--dag|--no-dag]')
        print('Example: python3 analyze_single_seed2.py 7476')
        sys.exit(1)
    ...
    print(f'\n{"="*70}')
    print('Single Seed Deep Dive Analysis for E21 (B1 Fixed Scenario)')
    print(f'{"="*70}')
    print(f'Seed: {seed}')
    print('B1 target: 0 < NCCR_total < 1 AND CCR_data > IDR_image')
    ...
    try:
        ...
        if ccr_data is None or idr_image is None or nccr_total is None:
            ...
            b1_match = (nccr_total > 0.0) and (nccr_total < 1.0) and (ccr_data > idr_image)
            print(f'B1 match (0 < NCCR < 1 and CCR > IDR): {b1_match}')
