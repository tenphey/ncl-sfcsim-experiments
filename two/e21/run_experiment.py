#!/usr/bin/env python3
"""
E21 Experiment: Fixed B1 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e21/b1.properties,
which targets: 0 < NCCR_total < 1 AND CCR_data > IDR_image.

Usage:
  python3 run_experiment.py                    # Normal run
  E21_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E21_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b1.properties")
...
DRY_RUN = int(os.getenv("E21_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E21_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E21_MASTER_SEED", "150"))
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = int(os.getenv("E21_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e21_results.csv")
    results = []
    ...
    print("=== E21 Experiment: Fixed B1 Scenario (No Variable Sweep) ===")
    print("Target scenario: 0 < NCCR_total < 1 AND CCR_data > IDR_image")
    ...
    # Write manifest
    manifest = {
        "experiment": "E21 - Fixed B1 Scenario (No Variable Sweep)",
        "scenario": "0 < NCCR_total < 1 AND CCR_data > IDR_image",
        "timestamp": timestamp,
    ...
    # Copy b1 properties snapshot
    with open(BASE_PROPS) as src:
        content = src.read()
    with open(os.path.join(out_dir, "b1_properties_snapshot.properties"), "w") as dst:
        dst.write(content)
