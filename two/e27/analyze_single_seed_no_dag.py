#!/usr/bin/env python3
"""
Single Seed Deep Dive Analysis for E27 (B7 Fixed Scenario)
...
【输出说明】
  ...
  - 该 seed 是否满足 b7 条件：NCCR_total > 2 AND CCR_data > IDR_image
...
def analyze():
    ...
    print(f'\n{"="*70}')
    print('Single Seed Deep Dive Analysis for E27 (B7 Fixed Scenario)')
    print(f'{"="*70}')
    print(f'Seed: {seed}')
    print('B7 target: NCCR_total > 2 AND CCR_data > IDR_image')
    print(f'{"="*70}\n')
    ...
    try:
        ...
        if ccr_data is None or idr_image is None or nccr_total is None:
            ...
            b7_match = (nccr_total > 2.0) and (ccr_data > idr_image)
            print('B7 match (NCCR > 2 and CCR_data > IDR_image): ' + str(b7_match))
