#!/usr/bin/env python3
"""
E27 Experiment: Fixed B7 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e27/b7.properties,
which targets: NCCR_total > 2 AND CCR_data > IDR_image.

Usage:
  python3 run_experiment.py                    # Normal run
  E27_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E27_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b7.properties")
...
DRY_RUN = int(os.getenv("E27_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E27_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E27_MASTER_SEED", "150"))
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = int(os.getenv("E27_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e27_results.csv")
    results = []
    ...
    print("=== E27 Experiment: Fixed B7 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E27 - Fixed B7 Scenario (No Variable Sweep)",
    ...
    # Copy b7 properties snapshot
    with open(BASE_PROPS) as src:
        content = src.read()
    with open(os.path.join(out_dir, "b7_properties_snapshot.properties"), "w") as dst:
        dst.write(content)
