#!/usr/bin/env python3
"""
Analyze E29 results for the fixed B9 scenario (no variable sweep).
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e29_results.csv")
    ...
    seed_metrics_csv = os.path.join(result_dir, "e29_seed_metrics_with_b9_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B9 condition
    df = df_algo_valid[df_algo_valid["b9_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print("No runs satisfy B9 condition: NCCR_total > 2 AND CCR_data < IDR_image")
    ...
    summary = {
        ...
        "b9_qualified_runs": qualified_runs,
        "b9_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e29_summary.csv")
    ...
    loss_json = {"b9": seed_map}
    loss_json_path = os.path.join(result_dir, "e29_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B9 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E29 Results (B9 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b9_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e29_summary_table.png")
    draw_table_image(table_df, "E29 Summary (B9 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e29_robust_table.png")
    draw_table_image(robust_print, "E29 Robust Statistics (B9 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E29 Mean Makespan Comparison (B9 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e29_makespan_mean_comparison.png")
    ...
    ax.set_title("E29 Makespan Distribution by Seed (B9 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e29_makespan_boxplot.png")
    ...
    ax.axvline(summary["gain_N_over_D_median"], color="#A23B92", linestyle="--", linewidth=2, label="median")
    ...
    ax.set_title("E29 Gain Distribution (B9 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e29_gain_histogram.png")
