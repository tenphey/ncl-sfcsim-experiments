#!/usr/bin/env python3
"""
E29 Experiment: Fixed B9 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e29/b9.properties,
which targets: NCCR_total > 2 AND CCR_data < IDR_image.

Usage:
  python3 run_experiment.py                    # Normal run
  E29_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E29_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
DRY_RUN = int(os.getenv("E29_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E29_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E29_MASTER_SEED", "150"))
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = int(os.getenv("E29_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e29_results.csv")
    results = []
    ...
    print("=== E29 Experiment: Fixed B9 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E29 - Fixed B9 Scenario (No Variable Sweep)",
    ...
    # Copy b9 properties snapshot
    with open(BASE_PROPS) as src:
        content = src.read()
    with open(os.path.join(out_dir, "b9_properties_snapshot.properties"), "w") as dst:
        dst.write(content)
