#!/usr/bin/env python3
"""
Analyze E25 results for the fixed B5 scenario (no variable sweep).
...
B5_ABS_TOL = float(os.getenv("E25_B5_ABS_TOL", "0.02"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e25_results.csv")
    ...
    df_all["b5_match"] = (
        (df_all["NCCR_total"] > 1.0)
        & (df_all["NCCR_total"] < 2.0)
        & (df_all["CCR_IDR_abs_diff"] <= B5_ABS_TOL)
    )

    seed_metrics_csv = os.path.join(result_dir, "e25_seed_metrics_with_b5_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B5 condition
    df = df_algo_valid[df_algo_valid["b5_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B5 condition: 1 < NCCR_total < 2 AND "
            f"|CCR_data-IDR_image| <= {B5_ABS_TOL:.4f}"
        )
    ...
    summary = {
        "scenario": "b5",
        "b5_abs_tolerance": B5_ABS_TOL,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b5_qualified_runs": qualified_runs,
        "b5_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e25_summary.csv")
    ...
    loss_json = {"b5": seed_map}
    loss_json_path = os.path.join(result_dir, "e25_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B5 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E25 Results (B5 Fixed Scenario) ===")
    wins_n_over_d_display = f"{wins_n_over_d}/{qualified_runs} ({summary['win_rate_N_over_D'] * 100:.2f}%)"
    console_row = {
        "scenario": "b5",
        "b5_abs_tol": B5_ABS_TOL,
        "algo_valid_runs": f"{algo_valid_runs}/{total_runs}",
        "b5_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e25_summary_table.png")
    draw_table_image(table_df, "E25 Summary (B5 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e25_robust_table.png")
    draw_table_image(robust_print, "E25 Robust Statistics (B5 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E25 Mean Makespan Comparison (B5 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e25_makespan_mean_comparison.png")
    ...
    ax.set_title("E25 Makespan Distribution by Seed (B5 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e25_makespan_boxplot.png")
    ...
    ax.axvline(summary["gain_N_over_D_median"], color="#A23B72", linestyle="--", linewidth=2, label="median")
    ...
    ax.set_title("E25 Gain Distribution (B5 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e25_gain_histogram.png")
