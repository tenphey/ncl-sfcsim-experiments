#!/usr/bin/env python3
"""
Single Seed Deep Dive Analysis for E25 (B5 Fixed Scenario)
...
【输出说明】
  ...
  - 该 seed 是否满足 b5 条件：1 < NCCR_total < 2 AND |CCR_data-IDR_image| <= tol
...
def analyze():
    ...
    print(f'\n{"="*70}')
    print('Single Seed Deep Dive Analysis for E25 (B5 Fixed Scenario)')
    print(f'{"="*70}')
    print(f'Seed: {seed}')
    print(f'B5 target: 1 < NCCR_total < 2 AND |CCR_data-IDR_image| <= {B5_ABS_TOL:.4f}')
    ...
    try:
        ...
        if ccr_data is None or idr_image is None or nccr_total is None:
            ...
            abs_diff = abs(ccr_data - idr_image)
            print(f'|CCR_data-IDR_image|: {abs_diff:.4f}')
            b5_match = (nccr_total > 1.0) and (nccr_total < 2.0) and (abs_diff <= B5_ABS_TOL)
            print(f'B5 match (1 < NCCR < 2 and |CCR-IDR| <= {B5_ABS_TOL:.4f}): {b5_match}')
