#!/usr/bin/env python3
"""
E25 Experiment: Fixed B5 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e25/b5.properties,
which targets: 1 < NCCR_total < 2 AND CCR_data ~= IDR_image.

Usage:
  python3 run_experiment.py                    # Normal run
  E25_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E25_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b5.properties")
...
DRY_RUN = int(os.getenv("E25_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E25_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E25_MASTER_SEED", "150"))
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = int(os.getenv("E25_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e25_results.csv")
    results = []
    ...
    print("=== E25 Experiment: Fixed B5 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E25 - Fixed B5 Scenario (No Variable Sweep)",
    ...
    # Copy b5 properties snapshot
    with open(BASE_PROPS) as src:
        content = src.read()
    with open(os.path.join(out_dir, "b5_properties_snapshot.properties"), "w") as dst:
        dst.write(content)
