#!/usr/bin/env python3
"""
Single Seed Deep Dive Analysis for E24 (B4 Fixed Scenario)
...
【输出说明】
  ...
  - 该 seed 是否满足 b4 条件：1 < NCCR_total < 2 AND CCR_data > IDR_image
...
def analyze():
    ...
    print(f'\n{"="*70}')
    print('Single Seed Deep Dive Analysis for E24 (B4 Fixed Scenario)')
    print(f'{"="*70}')
    print(f'Seed: {seed}')
    print('B4 target: 1 < NCCR_total < 2 AND CCR_data > IDR_image')
    print(f'{"="*70}\n')
    ...
    try:
        ...
        if ccr_data is None or idr_image is None or nccr_total is None:
            ...
            b4_match = (nccr_total > 1.0) and (nccr_total < 2.0) and (ccr_data > idr_image)
            print('B4 match (1 < NCCR < 2 and CCR_data > IDR_image): ' + str(b4_match))
