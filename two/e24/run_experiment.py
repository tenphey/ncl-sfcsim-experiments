#!/usr/bin/env python3
"""
E24 Experiment: Fixed B4 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e24/b4.properties,
which targets: 1 < NCCR_total < 2 AND CCR_data > IDR_image.

Usage:
  python3 run_experiment.py                    # Normal run
  E24_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E24_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b4.properties")
...
DRY_RUN = int(os.getenv("E24_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E24_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E24_MASTER_SEED", "150"))
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = int(os.getenv("E24_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e24_results.csv")
    results = []
    ...
    print("=== E24 Experiment: Fixed B4 Scenario (No Variable Sweep) ===")
    print("Target scenario: 1 < NCCR_total < 2 AND CCR_data > IDR_image")
    ...
    # Write manifest
    manifest = {
        "experiment": "E24 - Fixed B4 Scenario (No Variable Sweep)",
        "scenario": "1 < NCCR_total < 2 AND CCR_data > IDR_image",
        "timestamp": timestamp,
    ...
    # Copy b4 properties snapshot
    with open(BASE_PROPS) as src:
        content = src.read()
    with open(os.path.join(out_dir, "b4_properties_snapshot.properties"), "w") as dst:
        dst.write(content)
