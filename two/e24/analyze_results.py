#!/usr/bin/env python3
"""
Analyze E24 results for the fixed B4 scenario (no variable sweep).
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e24_results.csv")
    ...
    seed_metrics_csv = os.path.join(result_dir, "e24_seed_metrics_with_b4_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B4 condition
    df = df_algo_valid[df_algo_valid["b4_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print("No runs satisfy B4 condition: 1 < NCCR_total < 2 AND CCR_data > IDR_image")
    ...
    summary = {
        ...
        "b4_qualified_runs": qualified_runs,
        "b4_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e24_summary.csv")
    ...
    loss_json = {"b4": seed_map}
    loss_json_path = os.path.join(result_dir, "e24_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B4 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E24 Results (B4 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b4_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e24_summary_table.png")
    draw_table_image(table_df, "E24 Summary (B4 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e24_robust_table.png")
    draw_table_image(robust_print, "E24 Robust Statistics (B4 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E24 Mean Makespan Comparison (B4 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e24_makespan_mean_comparison.png")
    ...
    ax.set_title("E24 Makespan Distribution by Seed (B4 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e24_makespan_boxplot.png")
    ...
    ax.set_title("E24 Gain Distribution (B4 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e24_gain_histogram.png")
