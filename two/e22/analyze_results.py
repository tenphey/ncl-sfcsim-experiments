#!/usr/bin/env python3
"""
Analyze E22 results for the fixed B2 scenario (no variable sweep).
...
B2_ABS_TOL = float(os.getenv("E22_B2_ABS_TOL", "0.02"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e22_results.csv")
    ...
    df_all["b2_match"] = (
        (df_all["NCCR_total"] > 0.0)
        & (df_all["NCCR_total"] < 1.0)
        & (df_all["CCR_IDR_abs_diff"] <= B2_ABS_TOL)
    )

    seed_metrics_csv = os.path.join(result_dir, "e22_seed_metrics_with_b2_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B2 condition
    df = df_algo_valid[df_algo_valid["b2_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B2 condition: 0 < NCCR_total < 1 AND "
            f"|CCR_data-IDR_image| <= {B2_ABS_TOL:.4f}"
        )
    ...
    summary = {
        "scenario": "b2",
        "b2_abs_tolerance": B2_ABS_TOL,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b2_qualified_runs": qualified_runs,
        "b2_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e22_summary.csv")
    ...
    loss_json = {"b2": seed_map}
    loss_json_path = os.path.join(result_dir, "e22_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B2 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E22 Results (B2 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b2_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e22_summary_table.png")
    draw_table_image(table_df, "E22 Summary (B2 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e22_robust_table.png")
    draw_table_image(robust_print, "E22 Robust Statistics (B2 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E22 Mean Makespan Comparison (B2 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e22_makespan_mean_comparison.png")
    ...
    ax.set_title("E22 Makespan Distribution by Seed (B2 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e22_makespan_boxplot.png")
    ...
    ax.set_title("E22 Gain Distribution (B2 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e22_gain_histogram.png")
