#!/usr/bin/env python3
"""
Single Seed Deep Dive Analysis for E22 (B2 Fixed Scenario)
...
【输出说明】
  ...
  - 该 seed 是否满足 b2 条件：0 < NCCR_total < 1 AND |CCR_data-IDR_image| <= tol
...
def analyze():
    ...
    print(f'\n{"="*70}')
    print('Single Seed Deep Dive Analysis for E22 (B2 Fixed Scenario)')
    print(f'{"="*70}')
    print(f'Seed: {seed}')
    print(f'B2 target: 0 < NCCR_total < 1 AND |CCR_data-IDR_image| <= {B2_ABS_TOL:.4f}')
    print(f'{"="*70}\n')
    ...
    try:
        ...
        if ccr_data is None or idr_image is None or nccr_total is None:
            ...
            b2_match = (nccr_total > 0.0) and (nccr_total < 1.0) and (abs_diff <= B2_ABS_TOL)
            print(f'B2 match (0 < NCCR < 1 and |CCR-IDR| <= {B2_ABS_TOL:.4f}): {b2_match}')
