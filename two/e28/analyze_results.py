#!/usr/bin/env python3
"""
Analyze E28 results for the fixed B8 scenario (no variable sweep).
...
B8_ABS_TOL = float(os.getenv("E28_B8_ABS_TOL", "0.02"))
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e28_results.csv")
    ...
    df_all["b8_match"] = (
        ...
        & (df_all["CCR_IDR_abs_diff"] <= B8_ABS_TOL)
    )

    seed_metrics_csv = os.path.join(result_dir, "e28_seed_metrics_with_b8_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B8 condition
    df = df_algo_valid[df_algo_valid["b8_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print(
            f"No runs satisfy B8 condition: NCCR_total > 2 AND "
            f"|CCR_data-IDR_image| <= {B8_ABS_TOL:.4f}"
        )
    ...
    summary = {
        "scenario": "b8",
        "b8_abs_tolerance": B8_ABS_TOL,
        "total_runs": total_runs,
        "algo_valid_runs": algo_valid_runs,
        "b8_qualified_runs": qualified_runs,
        "b8_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e28_summary.csv")
    ...
    loss_json = {"b8": seed_map}
    loss_json_path = os.path.join(result_dir, "e28_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B8 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E28 Results (B8 Fixed Scenario) ===")
    wins_n_over_d_display = f"{wins_n_over_d}/{qualified_runs} ({summary['win_rate_N_over_D'] * 100:.2f}%)"
    console_row = {
        "scenario": "b8",
        "b8_abs_tol": B8_ABS_TOL,
        "algo_valid_runs": f"{algo_valid_runs}/{total_runs}",
        "b8_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e28_summary_table.png")
    draw_table_image(table_df, "E28 Summary (B8 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e28_robust_table.png")
    draw_table_image(robust_print, "E28 Robust Statistics (B8 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E28 Mean Makespan Comparison (B8 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e28_makespan_mean_comparison.png")
    ...
    ax.set_title("E28 Makespan Distribution by Seed (B8 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e28_makespan_boxplot.png")
    ...
    ax.set_title("E28 Gain Distribution (B8 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e28_gain_histogram.png")
