#!/usr/bin/env python3
"""
Single Seed Deep Dive Analysis for E28 (B8 Fixed Scenario)
...
【输出说明】
  ...
  - 该 seed 是否满足 b8 条件：NCCR_total > 2 AND |CCR_data-IDR_image| <= tol
...
def analyze():
    ...
    print(f'\n{"="*70}')
    print('Single Seed Deep Dive Analysis for E28 (B8 Fixed Scenario)')
    print(f'{"="*70}')
    print(f'Seed: {seed}')
    print(f'B8 target: NCCR_total > 2 AND |CCR_data-IDR_image| <= {B8_ABS_TOL:.4f}')
    ...
    try:
        ...
        if ccr_data is None or idr_image is None or nccr_total is None:
            ...
            b8_match = (nccr_total > 2.0) and (abs_diff <= B8_ABS_TOL)
            print(f'B8 match (NCCR > 2 and |CCR-IDR| <= {B8_ABS_TOL:.4f}): {b8_match}')
