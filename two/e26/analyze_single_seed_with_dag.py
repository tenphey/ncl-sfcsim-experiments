#!/usr/bin/env python3
"""
Single Seed Deep Dive Analysis for E26 (B6 Fixed Scenario)
...
【输出说明】
  ...
  - 该 seed 是否满足 b6 条件：1 < NCCR_total < 2 AND CCR_data < IDR_image
...
def analyze():
    ...
    print(f'\n{"="*70}')
    print('Single Seed Deep Dive Analysis for E26 (B6 Fixed Scenario)')
    print(f'{"="*70}')
    print(f'Seed: {seed}')
    print('B6 target: 1 < NCCR_total < 2 AND CCR_data < IDR_image')
    ...
    try:
        ...
        if ccr_data is None or idr_image is None or nccr_total is None:
            ...
            b6_match = (nccr_total > 1.0) and (nccr_total < 2.0) and (ccr_data < idr_image)
            print('B6 match (1 < NCCR < 2 and CCR_data < IDR_image): ' + str(b6_match))
