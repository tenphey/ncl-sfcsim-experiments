#!/usr/bin/env python3
"""
Analyze E26 results for the fixed B6 scenario (no variable sweep).
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e26_results.csv")
    ...
    seed_metrics_csv = os.path.join(result_dir, "e26_seed_metrics_with_b6_flag.csv")
    ...
    # Step 2: keep only seeds that satisfy B6 condition
    df = df_algo_valid[df_algo_valid["b6_match"]].copy()
    qualified_runs = len(df)
    if qualified_runs == 0:
        print("No runs satisfy B6 condition: 1 < NCCR_total < 2 AND CCR_data < IDR_image")
    ...
    summary = {
        ...
        "b6_qualified_runs": qualified_runs,
        "b6_qualified_rate_in_algo_valid": qualified_runs / algo_valid_runs if algo_valid_runs > 0 else np.nan,
    ...
    summary_csv = os.path.join(result_dir, "grid_e26_summary.csv")
    ...
    loss_json = {"b6": seed_map}
    loss_json_path = os.path.join(result_dir, "e26_nheft_loss_seeds_by_scenario.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_json, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print(f"Seed metrics with B6 flag: {seed_metrics_csv}")

    # Console/table summary
    print("\n=== E26 Results (B6 Fixed Scenario) ===")
    ...
    console_row = {
        ...
        "b6_qualified_runs": f"{qualified_runs}/{algo_valid_runs}",
    ...
    summary_table_path = os.path.join(result_dir, "e26_summary_table.png")
    draw_table_image(table_df, "E26 Summary (B6 Fixed Scenario)", summary_table_path)
    ...
    robust_table_path = os.path.join(result_dir, "e26_robust_table.png")
    draw_table_image(robust_print, "E26 Robust Statistics (B6 Fixed Scenario)", robust_table_path)
    ...
    ax.set_title("E26 Mean Makespan Comparison (B6 Fixed Scenario)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    chart_means = os.path.join(result_dir, "e26_makespan_mean_comparison.png")
    ...
    ax.set_title("E26 Makespan Distribution by Seed (B6 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)

    plt.tight_layout()
    chart_box = os.path.join(result_dir, "e26_makespan_boxplot.png")
    ...
    ax.set_title("E26 Gain Distribution (B6 Fixed Scenario)", fontsize=13, fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax.legend(loc="best")

    plt.tight_layout()
    chart_hist = os.path.join(result_dir, "e26_gain_histogram.png")
