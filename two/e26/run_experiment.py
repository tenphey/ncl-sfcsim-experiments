#!/usr/bin/env python3
"""
E26 Experiment: Fixed B6 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e26/b6.properties,
which targets: 1 < NCCR_total < 2 AND CCR_data < IDR_image.

Usage:
  python3 run_experiment.py                    # Normal run
  E26_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E26_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
BASE_PROPS = os.path.join(THIS_DIR, "b6.properties")
...
DRY_RUN = int(os.getenv("E26_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E26_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E26_MASTER_SEED", "150"))
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = int(os.getenv("E26_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e26_results.csv")
    results = []
    ...
    print("=== E26 Experiment: Fixed B6 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E26 - Fixed B6 Scenario (No Variable Sweep)",
    ...
    # Copy b6 properties snapshot
    with open(BASE_PROPS) as src:
        content = src.read()
    with open(os.path.join(out_dir, "b6_properties_snapshot.properties"), "w") as dst:
        dst.write(content)
