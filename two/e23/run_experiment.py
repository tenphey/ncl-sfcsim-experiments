#!/usr/bin/env python3
"""
E23 Experiment: Fixed B3 Scenario (No Variable Sweep)

Runs repeated random-seed trials on experiments/e23/b3.properties,
which targets: 0 < NCCR_total < 1 AND CCR_data < IDR_image.

Usage:
  python3 run_experiment.py                    # Normal run
  E23_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E23_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
DRY_RUN = int(os.getenv("E23_DRY_RUN", "0")) == 1
LIMIT_RUNS = int(os.getenv("E23_LIMIT_RUNS", "0"))

# Seeding
MASTER_SEED = int(os.getenv("E23_MASTER_SEED", "150"))
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = int(os.getenv("E23_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e23_results.csv")
    results = []
    ...
    print("=== E23 Experiment: Fixed B3 Scenario (No Variable Sweep) ===")
    ...
    # Write manifest
    manifest = {
        "experiment": "E23 - Fixed B3 Scenario (No Variable Sweep)",
    ...
    # Copy b3 properties snapshot
    with open(BASE_PROPS) as src:
        content = src.read()
    with open(os.path.join(out_dir, "b3_properties_snapshot.properties"), "w") as dst:
        dst.write(content)
