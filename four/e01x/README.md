# E01X: Shared Baseline Scenario Run
...
```bash
E01X_NUM_SEEDS=1 E01X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e01x/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e01x/run_experiment.py
```

Dry run:

```bash
E01X_NUM_SEEDS=1 E01X_LIMIT_RUNS=1 E01X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e01x/run_experiment.py
```

Optional override:

```bash
E01X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e01x` as the common baseline side
