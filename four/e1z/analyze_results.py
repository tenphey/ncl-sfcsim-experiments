#!/usr/bin/env python3
"""Aggregate all E1Z bucket results into one cross-bucket overview.

This script combines the latest raw results of:

  e11z, e12z, e13z, e14z, e15z, e16z, e17z, e18z
...
Usage from the simulator repository root:
  ...
  experiments/.venv/bin/python experiments/four/e1z/analyze_results.py
"""
...
MIN_REL_GAP_PCT = float(os.getenv("E1Z_MIN_REL_GAP_PCT", "20.0"))

BASE_VARIANT = "baseline"
GATE_VARIANT = "comp_advantage"

BUCKET_SPECS = [
    ("e11z", "(0.10, 0.18]", 0.10, 0.18),
    ("e12z", "(0.18, 0.32]", 0.18, 0.32),
    ("e13z", "(0.32, 0.56]", 0.32, 0.56),
    ("e14z", "(0.56, 1.00]", 0.56, 1.00),
    ("e15z", "(1.00, 1.78]", 1.00, 1.78),
    ("e16z", "(1.78, 3.16]", 1.78, 3.16),
    ("e17z", "(3.16, 5.62]", 3.16, 5.62),
    ("e18z", "(5.62, 10.00]", 5.62, 10.00),
]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Aggregate e11z-e18z into one z-condition overview figure."
    )
    parser.add_argument(
        "--output-dir",
        help="Optional output directory. Default: experiments/four/e1z/analysis_<timestamp>",
    )
...
def validate_filtered_rows(filtered, lower, upper):
    """Defensive guard for z-condition aggregation.
    ...
    if not bad_nccr.empty:
        raise RuntimeError(
            ...
            "which should never happen in e1z aggregation"
        )
    ...
    if not bad_gap.empty:
        raise RuntimeError(
            "Filtered rows violate the z-condition relative CCR/IDR gap "
...
def expected_variants_from_manifest(manifest, df):
    ...
    if GATE_VARIANT not in labels:
        raise RuntimeError(
            f"The {GATE_VARIANT} variant is required for e1z aggregation"
        )
    if len(labels) != 2:
        raise RuntimeError(
            "e1z aggregation expects exactly two variants: "
...
def build_bucket_summary(
    ...
    return {
        ...
        "matched_ccr_minus_idr_min": float(complete_df["ccr_minus_idr"].min()),
...
def save_overview_plot(summary_df, output_dir):
    ...
    ax_m.set_title("E1Z overview: makespan lines and used-vCPU bars across 8 buckets")

    fig.tight_layout()
    output_path = os.path.join(output_dir, "e1z_makespan_vcpu_overview.png")
...
def write_seed_coverage_md(output_dir, summary_df):
    output_path = os.path.join(output_dir, "e1z_seed_coverage_report.md")
    lines = [
        ...
        (
            ...
            f"and the z-condition (`CCR_data > IDR_image` and relative CCR/IDR gap >= {MIN_REL_GAP_PCT:.1f}%)."
        ),
        (
...
def write_manifest(output_dir, summary_df):
    payload = {
        "analysis": "E1Z cross-bucket aggregation",
        "timestamp": datetime.now().strftime("%Y%m%d_%H%M%S"),
        "bucket_count": int(len(summary_df)),
        "min_rel_gap_pct": MIN_REL_GAP_PCT,
        "note": (
            "Each bucket uses only complete seeds after z-condition filtering. "
...
def main():
    ...
    summary_csv = os.path.join(output_dir, "e1z_bucket_summary.csv")
    seed_report_csv = os.path.join(output_dir, "e1z_complete_seed_report.csv")
    ...
    print("E1Z cross-bucket aggregation complete.")
