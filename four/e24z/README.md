# E24Z: Fourth NCCR Bucket, Data-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e24z/b24z.properties
```
...
```bash
E24Z_NUM_SEEDS=1 E24Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e24z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e24z/run_experiment.py
```

Dry run:

```bash
E24Z_NUM_SEEDS=1 E24Z_LIMIT_RUNS=1 E24Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e24z/run_experiment.py
```
...
```bash
E24Z_NUM_SEEDS=20
E24Z_MASTER_SEED=151
E24Z_TIMEOUT=180
E24Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e24z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e24z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E24Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e24z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e24z_seed_variant_paired_metrics.csv`
- `e24z_variant_summary.csv`
- `e24z_reference_summary.csv`
- `e24z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e24z` asks the
