#!/usr/bin/env python3
"""Analyze E15X NHEFT computation-advantage gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E15X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e15x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E15X comp-advantage result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E15X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E15X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E15X_ABS_TOL", "0.02"))
...
def filter_scenario_rows(ok):
    ...
    scenario_ok["scenario_match"] = (
        (scenario_ok["nccr_total"] > 1.00)
        & (scenario_ok["nccr_total"] <= 1.78)
        & (scenario_ok["ccr_idr_abs_diff"] <= BALANCED_ABS_TOL)
    )

    scenario_rule = (
        f"1.00 < NCCR_total <= 1.78 AND |CCR_data - IDR_image| <= {BALANCED_ABS_TOL:.4f}"
    )
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E15X NHEFT computation-advantage gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E15X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E15X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e15x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e15x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e15x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e15x_complete_seed_report.csv")
