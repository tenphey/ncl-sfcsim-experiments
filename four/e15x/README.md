# E15X: Fifth NCCR Bucket, Balanced CCR/IDR / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e15x/b15x.properties
```
...
```bash
E15X_NUM_SEEDS=1 E15X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e15x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e15x/run_experiment.py
```

Dry run:

```bash
E15X_NUM_SEEDS=1 E15X_LIMIT_RUNS=1 E15X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e15x/run_experiment.py
```
...
```bash
E15X_NUM_SEEDS=20
E15X_MASTER_SEED=151
E15X_TIMEOUT=180
E15X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e15x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e15x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
1.00 < NCCR_total <= 1.78 AND |CCR_data - IDR_image| <= 0.02
```

Additional useful analysis override:

```bash
E15X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e15x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e15x_seed_variant_paired_metrics.csv`
- `e15x_variant_summary.csv`
- `e15x_reference_summary.csv`
- `e15x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e15x` asks the
