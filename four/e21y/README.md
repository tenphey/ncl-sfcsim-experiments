# E21Y: First NCCR Bucket, Image-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e21y/b21y.properties
```
...
```bash
E21Y_NUM_SEEDS=1 E21Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e21y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e21y/run_experiment.py
```

Dry run:

```bash
E21Y_NUM_SEEDS=1 E21Y_LIMIT_RUNS=1 E21Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e21y/run_experiment.py
```
...
```bash
E21Y_NUM_SEEDS=20
E21Y_MASTER_SEED=151
E21Y_TIMEOUT=180
E21Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e21y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e21y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E21Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e21y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e21y_seed_variant_paired_metrics.csv`
- `e21y_variant_summary.csv`
- `e21y_reference_summary.csv`
- `e21y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e21y` asks the
