# E02X: Shared Baseline Scenario Run
...
```bash
E02X_NUM_SEEDS=1 E02X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e02x/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e02x/run_experiment.py
```

Dry run:

```bash
E02X_NUM_SEEDS=1 E02X_LIMIT_RUNS=1 E02X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e02x/run_experiment.py
```

Optional override:

```bash
E02X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e02x` as the common baseline side
