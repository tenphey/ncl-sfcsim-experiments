#!/usr/bin/env python3
"""Aggregate all E1X bucket results into one cross-bucket overview.

This script combines the latest raw results of:

  e11x, e12x, e13x, e14x, e15x, e16x, e17x, e18x
...
Usage from the simulator repository root:
  ...
  experiments/.venv/bin/python experiments/four/e1x/analyze_results.py
"""
...
BALANCED_ABS_TOL = float(os.getenv("E1X_ABS_TOL", "0.02"))

BASE_VARIANT = "baseline"
GATE_VARIANT = "comp_advantage"

BUCKET_SPECS = [
    ("e11x", "(0.10, 0.18]", 0.10, 0.18),
    ("e12x", "(0.18, 0.32]", 0.18, 0.32),
    ("e13x", "(0.32, 0.56]", 0.32, 0.56),
    ("e14x", "(0.56, 1.00]", 0.56, 1.00),
    ("e15x", "(1.00, 1.78]", 1.00, 1.78),
    ("e16x", "(1.78, 3.16]", 1.78, 3.16),
    ("e17x", "(3.16, 5.62]", 3.16, 5.62),
    ("e18x", "(5.62, 10.00]", 5.62, 10.00),
]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Aggregate e11x-e18x into one x-condition overview figure."
    )
    parser.add_argument(
        "--output-dir",
        help="Optional output directory. Default: experiments/four/e1x/analysis_<timestamp>",
    )
...
def filter_bucket_rows(ok, lower, upper):
    ...
    scenario_ok["scenario_match"] = (
        ...
        & (scenario_ok["ccr_idr_abs_diff"] <= BALANCED_ABS_TOL)
    )
    ...
    if filtered.empty:
        raise RuntimeError(
            ...
            f"AND |CCR_data - IDR_image| <= {BALANCED_ABS_TOL:.4f}"
        )
...
def validate_filtered_rows(filtered, lower, upper):
    """Defensive guard for x-condition aggregation.

    The overview script must never summarize rows outside:
      lower < NCCR_total <= upper
      AND |CCR_data - IDR_image| <= BALANCED_ABS_TOL
    """
    bad_nccr = filtered[
        ~(
            (filtered["nccr_total"] > lower)
            & (filtered["nccr_total"] <= upper)
        )
    ]
    if not bad_nccr.empty:
        raise RuntimeError(
            "Filtered rows contain out-of-bucket NCCR values, "
            "which should never happen in e1x aggregation"
        )

    bad_balance = filtered[filtered["ccr_idr_abs_diff"] > BALANCED_ABS_TOL]
...
def expected_variants_from_manifest(manifest, df):
    ...
    if GATE_VARIANT not in labels:
        raise RuntimeError(
            f"The {GATE_VARIANT} variant is required for e1x aggregation"
        )
    if len(labels) != 2:
        raise RuntimeError(
            "e1x aggregation expects exactly two variants: "
...
def build_bucket_summary(
    ...
    return {
        ...
        "matched_abs_diff_max": float(complete_df["ccr_idr_abs_diff"].max()),
...
def save_overview_plot(summary_df, output_dir):
    ...
    ax_m.set_title("E1X overview: makespan lines and used-vCPU bars across 8 buckets")

    fig.tight_layout()
    output_path = os.path.join(output_dir, "e1x_makespan_vcpu_overview.png")
...
def write_seed_coverage_md(output_dir, summary_df):
    output_path = os.path.join(output_dir, "e1x_seed_coverage_report.md")
    lines = [
...
def write_manifest(output_dir, summary_df):
    payload = {
        "analysis": "E1X cross-bucket aggregation",
        "timestamp": datetime.now().strftime("%Y%m%d_%H%M%S"),
        "bucket_count": int(len(summary_df)),
        "balanced_abs_tol": BALANCED_ABS_TOL,
        "note": (
            "Each bucket uses only complete seeds after x-condition filtering. "
...
def main():
    ...
    summary_csv = os.path.join(output_dir, "e1x_bucket_summary.csv")
    seed_report_csv = os.path.join(output_dir, "e1x_complete_seed_report.csv")
    ...
    print("E1X cross-bucket aggregation complete.")
