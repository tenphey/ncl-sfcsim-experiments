# E26Y: Sixth NCCR Bucket, Image-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e26y/b26y.properties
```
...
```bash
E26Y_NUM_SEEDS=1 E26Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e26y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e26y/run_experiment.py
```

Dry run:

```bash
E26Y_NUM_SEEDS=1 E26Y_LIMIT_RUNS=1 E26Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e26y/run_experiment.py
```
...
```bash
E26Y_NUM_SEEDS=20
E26Y_MASTER_SEED=151
E26Y_TIMEOUT=180
E26Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e26y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e26y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E26Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e26y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e26y_seed_variant_paired_metrics.csv`
- `e26y_variant_summary.csv`
- `e26y_reference_summary.csv`
- `e26y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e26y` asks the
