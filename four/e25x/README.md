# E25X: Fifth NCCR Bucket, Balanced CCR/IDR / NHEFT Computation + DRT Gate
...
```text
experiments/four/e25x/b25x.properties
```
...
```bash
E25X_NUM_SEEDS=1 E25X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e25x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e25x/run_experiment.py
```

Dry run:

```bash
E25X_NUM_SEEDS=1 E25X_LIMIT_RUNS=1 E25X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e25x/run_experiment.py
```
...
```bash
E25X_NUM_SEEDS=20
E25X_MASTER_SEED=151
E25X_TIMEOUT=180
E25X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e25x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e25x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E25X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e25x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e25x_seed_variant_paired_metrics.csv`
- `e25x_variant_summary.csv`
- `e25x_reference_summary.csv`
- `e25x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e25x` asks the
