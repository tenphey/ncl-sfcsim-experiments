# E17Y: Seventh NCCR Bucket, Image-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e17y/b17y.properties
```
...
```bash
E17Y_NUM_SEEDS=1 E17Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e17y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e17y/run_experiment.py
```

Dry run:

```bash
E17Y_NUM_SEEDS=1 E17Y_LIMIT_RUNS=1 E17Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e17y/run_experiment.py
```
...
```bash
experiments/.venv/bin/python   experiments/four/e17y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e17y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E17Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e17y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e17y_seed_variant_paired_metrics.csv`
- `e17y_variant_summary.csv`
- `e17y_reference_summary.csv`
- `e17y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e17y` asks the
