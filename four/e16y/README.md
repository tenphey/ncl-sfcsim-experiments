# E16Y: Sixth NCCR Bucket, Image-Dominant / NHEFT Computation-Advantage Gate
...
```text
1.78 < NCCR_total <= 3.16 AND CCR_data < IDR_image
```

The parameter values are reused from:

```text
experiments/three/e46
```

so that the `four/` gate study can be tested under the same tuned sixth-bucket
...
```text
experiments/four/e16y/b16y.properties
```
...
sixth-bucket scenario, while the same seed is reused for both baseline and
comp-gated NHEFT.
...
```bash
E16Y_NUM_SEEDS=1 E16Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e16y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e16y/run_experiment.py
```

Dry run:

```bash
E16Y_NUM_SEEDS=1 E16Y_LIMIT_RUNS=1 E16Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e16y/run_experiment.py
```
...
```bash
E16Y_NUM_SEEDS=20
E16Y_MASTER_SEED=151
E16Y_TIMEOUT=180
E16Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e16y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e16y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
1.78 < NCCR_total <= 3.16 AND CCR_data < IDR_image AND relative CCR/IDR gap >= 20%
```

Additional useful analysis override:

```bash
E16Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e16y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e16y_seed_variant_paired_metrics.csv`
- `e16y_variant_summary.csv`
- `e16y_reference_summary.csv`
- `e16y_complete_seed_report.csv`
...
- under this **one fixed sixth-bucket communication condition**, does the
  computation-advantage gate reduce vCPU usage while keeping more makespan
  benefit than DHEFT?

In other words, `e1` asks the general gate question, while `e16y` asks the
