# E13Y: Third NCCR Bucket, Image-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e13y/b13y.properties
```
...
```bash
E13Y_NUM_SEEDS=1 E13Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e13y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e13y/run_experiment.py
```

Dry run:

```bash
E13Y_NUM_SEEDS=1 E13Y_LIMIT_RUNS=1 E13Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e13y/run_experiment.py
```
...
```bash
experiments/.venv/bin/python   experiments/four/e13y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e13y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
0.32 < NCCR_total <= 0.56 AND CCR_data < IDR_image AND relative CCR/IDR gap >= 20%
```

Additional useful analysis override:

```bash
E13Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e13y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e13y_seed_variant_paired_metrics.csv`
- `e13y_variant_summary.csv`
- `e13y_reference_summary.csv`
- `e13y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e13y` asks the
