# E23Y: Third NCCR Bucket, Image-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e23y/b23y.properties
```
...
```bash
E23Y_NUM_SEEDS=1 E23Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e23y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e23y/run_experiment.py
```

Dry run:

```bash
E23Y_NUM_SEEDS=1 E23Y_LIMIT_RUNS=1 E23Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e23y/run_experiment.py
```
...
```bash
E23Y_NUM_SEEDS=20
E23Y_MASTER_SEED=151
E23Y_TIMEOUT=180
E23Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e23y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e23y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E23Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e23y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e23y_seed_variant_paired_metrics.csv`
- `e23y_variant_summary.csv`
- `e23y_reference_summary.csv`
- `e23y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e23y` asks the
