# E25Y: Fifth NCCR Bucket, Image-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e25y/b25y.properties
```
...
```bash
E25Y_NUM_SEEDS=1 E25Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e25y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e25y/run_experiment.py
```

Dry run:

```bash
E25Y_NUM_SEEDS=1 E25Y_LIMIT_RUNS=1 E25Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e25y/run_experiment.py
```
...
```bash
E25Y_NUM_SEEDS=20
E25Y_MASTER_SEED=151
E25Y_TIMEOUT=180
E25Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e25y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e25y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E25Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e25y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e25y_seed_variant_paired_metrics.csv`
- `e25y_variant_summary.csv`
- `e25y_reference_summary.csv`
- `e25y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e25y` asks the
