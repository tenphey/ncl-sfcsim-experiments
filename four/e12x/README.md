# E12X: Second NCCR Bucket, Balanced CCR/IDR / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e12x/b12x.properties
```
...
```bash
E12X_NUM_SEEDS=1 E12X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e12x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e12x/run_experiment.py
```

Dry run:

```bash
E12X_NUM_SEEDS=1 E12X_LIMIT_RUNS=1 E12X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e12x/run_experiment.py
```
...
```bash
experiments/.venv/bin/python   experiments/four/e12x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e12x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
0.18 < NCCR_total <= 0.32 AND |CCR_data - IDR_image| <= 0.02
```

Additional useful analysis override:

```bash
E12X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e12x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e12x_seed_variant_paired_metrics.csv`
- `e12x_variant_summary.csv`
- `e12x_reference_summary.csv`
- `e12x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e12x` asks the
