# E24Y: Fourth NCCR Bucket, Image-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e24y/b24y.properties
```
...
```bash
E24Y_NUM_SEEDS=1 E24Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e24y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e24y/run_experiment.py
```

Dry run:

```bash
E24Y_NUM_SEEDS=1 E24Y_LIMIT_RUNS=1 E24Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e24y/run_experiment.py
```
...
```bash
E24Y_NUM_SEEDS=20
E24Y_MASTER_SEED=151
E24Y_TIMEOUT=180
E24Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e24y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e24y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E24Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e24y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e24y_seed_variant_paired_metrics.csv`
- `e24y_variant_summary.csv`
- `e24y_reference_summary.csv`
- `e24y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e24y` asks the
