#!/usr/bin/env python3
"""Analyze E18X NHEFT computation-advantage gate results.
...
- comp_advantage: NHEFT with the computation-advantage gate enabled

If a run was stopped in the middle of a seed, that incomplete seed is excluded
from all summaries and plots.  A seed is used only when it has all expected E18X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e18x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E18X comp-advantage result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E18X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E18X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E18X_ABS_TOL", "0.02"))
...
def filter_scenario_rows(ok):
    ...
    scenario_ok["scenario_match"] = (
        (scenario_ok["nccr_total"] > 5.62)
        & (scenario_ok["nccr_total"] <= 10.00)
        & (scenario_ok["ccr_idr_abs_diff"] <= BALANCED_ABS_TOL)
    )

    scenario_rule = (
        f"5.62 < NCCR_total <= 10.00 AND |CCR_data - IDR_image| <= {BALANCED_ABS_TOL:.4f}"
    )
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E18X NHEFT computation-advantage gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E18X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E18X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e18x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e18x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e18x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e18x_complete_seed_report.csv")
