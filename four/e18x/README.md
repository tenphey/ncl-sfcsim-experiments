# E18X: Eighth NCCR Bucket, Balanced CCR/IDR / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e18x/b18x.properties
```
...
```bash
E18X_NUM_SEEDS=1 E18X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e18x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e18x/run_experiment.py
```

Dry run:

```bash
E18X_NUM_SEEDS=1 E18X_LIMIT_RUNS=1 E18X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e18x/run_experiment.py
```
...
```bash
E18X_NUM_SEEDS=20
E18X_MASTER_SEED=151
E18X_TIMEOUT=180
E18X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e18x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e18x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
5.62 < NCCR_total <= 10.00 AND |CCR_data - IDR_image| <= 0.02
```

Additional useful analysis override:

```bash
E18X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e18x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e18x_seed_variant_paired_metrics.csv`
- `e18x_variant_summary.csv`
- `e18x_reference_summary.csv`
- `e18x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e18x` asks the
