# E16Z: Sixth NCCR Bucket, Data-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e16z/b16z.properties
```
...
```bash
E16Z_NUM_SEEDS=1 E16Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e16z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e16z/run_experiment.py
```

Dry run:

```bash
E16Z_NUM_SEEDS=1 E16Z_LIMIT_RUNS=1 E16Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e16z/run_experiment.py
```
...
```bash
E16Z_NUM_SEEDS=20
E16Z_MASTER_SEED=151
E16Z_TIMEOUT=180
E16Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e16z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e16z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E16Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e16z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e16z_seed_variant_paired_metrics.csv`
- `e16z_variant_summary.csv`
- `e16z_reference_summary.csv`
- `e16z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e16z` asks the
