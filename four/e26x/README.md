# E26X: Sixth NCCR Bucket, Balanced CCR/IDR / NHEFT Computation + DRT Gate
...
```text
experiments/four/e26x/b26x.properties
```
...
```bash
E26X_NUM_SEEDS=1 E26X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e26x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e26x/run_experiment.py
```

Dry run:

```bash
E26X_NUM_SEEDS=1 E26X_LIMIT_RUNS=1 E26X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e26x/run_experiment.py
```
...
```bash
E26X_NUM_SEEDS=20
E26X_MASTER_SEED=151
E26X_TIMEOUT=180
E26X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e26x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e26x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E26X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e26x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e26x_seed_variant_paired_metrics.csv`
- `e26x_variant_summary.csv`
- `e26x_reference_summary.csv`
- `e26x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e26x` asks the
