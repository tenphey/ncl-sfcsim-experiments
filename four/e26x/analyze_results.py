#!/usr/bin/env python3
"""Analyze E26X NHEFT comp+DRT gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E26X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e26x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E26X comp+DRT result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E26X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E26X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E26X_ABS_TOL", "0.02"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E26X NHEFT comp+DRT gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E26X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E26X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e26x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e26x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e26x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e26x_complete_seed_report.csv")
