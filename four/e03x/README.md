# E03X: Shared Baseline Scenario Run
...
```text
experiments/four/e03x/b03x.properties
```
...
```bash
E03X_NUM_SEEDS=1 E03X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e03x/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e03x/run_experiment.py
```

Dry run:

```bash
E03X_NUM_SEEDS=1 E03X_LIMIT_RUNS=1 E03X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e03x/run_experiment.py
```

Optional override:

```bash
E03X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e03x` as the common baseline side
