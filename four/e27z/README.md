# E27Z: Seventh NCCR Bucket, Data-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e27z/b27z.properties
```
...
```bash
E27Z_NUM_SEEDS=1 E27Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e27z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e27z/run_experiment.py
```

Dry run:

```bash
E27Z_NUM_SEEDS=1 E27Z_LIMIT_RUNS=1 E27Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e27z/run_experiment.py
```
...
```bash
E27Z_NUM_SEEDS=20
E27Z_MASTER_SEED=151
E27Z_TIMEOUT=180
E27Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e27z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e27z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E27Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e27z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e27z_seed_variant_paired_metrics.csv`
- `e27z_variant_summary.csv`
- `e27z_reference_summary.csv`
- `e27z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e27z` asks the
