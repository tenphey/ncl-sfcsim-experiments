#!/usr/bin/env python3
"""Analyze E3 NHEFT IRT-only gate results.
...
- irt_only: NHEFT with comp and IRT opening gates enabled

If a run was stopped in the middle of a seed, that incomplete seed is excluded
from all summaries and plots.  A seed is used only when it has all expected E3
variants.
...
RAW_CSV_NAME = "e3_irt_only_results.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E3 IRT-only gate result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
        help=(
            ...
            "with e3_irt_only_results.csv is used."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E3 paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E3 analysis")
        ...
        rows.append(
            {
                ...
                "configured_drt_advantage_mean": group[
                    "configured_drt_advantage"
                ].mean(),
                "configured_irt_advantage_mean": group[
                    "configured_irt_advantage"
                ].mean(),
...
def save_summary_table(summary, output_dir):
    display_cols = [
        ...
        "configured_drt_advantage_mean",
        "configured_irt_advantage_mean",
        "nheft_makespan_mean",
    ...
    display = display.rename(
        columns={
            ...
            "configured_drt_advantage_mean": "drt gate",
            "configured_irt_advantage_mean": "irt gate",
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E3 NHEFT IRT-only gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E3 variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E3 result folder found.")
    ...
    complete_ok_df, complete_seed_report = filter_complete_seeds(ok_df, expected_variants)
    ...
    paired_csv = os.path.join(output_dir, "e3_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e3_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e3_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e3_complete_seed_report.csv")
