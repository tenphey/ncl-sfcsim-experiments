#!/usr/bin/env python3
"""Generate paired data for the NHEFT comp+IRT gate experiment.
...
Run from the simulator repository root, for example:
  ...
  experiments/four/e3_nheft_comp_irt_gates/run_experiment.py

Useful environment variables:

  E3_NUM_SEEDS=5       Run five seeds instead of the default 1000.
  E3_MASTER_SEED=151   Change the deterministic seed list.
  E3_LIMIT_RUNS=2      Stop after two Java invocations (smoke testing).
  E3_DRY_RUN=1         Print commands without launching Java.
  E3_TIMEOUT=180       Timeout in seconds for each Java invocation.
  E3_BASE_PROPERTIES=/absolute/path/to/file.properties
...
MASTER_SEED = int(os.getenv("E3_MASTER_SEED", "151"))
NUM_SEEDS = int(os.getenv("E3_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
LIMIT_RUNS = int(os.getenv("E3_LIMIT_RUNS", "0"))
DRY_RUN = os.getenv("E3_DRY_RUN", "0") == "1"
TIMEOUT = int(os.getenv("E3_TIMEOUT", "180"))
RAW_CSV_NAME = "e3_comp_irt_results.csv"
...
GATE_VARIANT = {
    "label": "comp_irt",
    "display_label": "NHEFT+CompIRT",
    "tolerance": "0.0",
    "comp_advantage": "1",
    "drt_advantage": "0",
    "irt_advantage": "1",
    "description": "Open a new vCPU only when it has earlier EFT and simultaneously has comp and IRT advantages over the best already-used vCPU.",
}
...
def resolve_base_properties(project_root):
    override = os.getenv("E3_BASE_PROPERTIES")
    if override:
...
def make_seed_list():
    if NUM_SEEDS <= 0:
        raise ValueError("E3_NUM_SEEDS must be positive")
    population = range(1000, 1000000)
    if NUM_SEEDS > len(population):
        raise ValueError("E3_NUM_SEEDS is larger than the available seed range")
...
def main():
    ...
    manifest = {
        "experiment": "E3 - NHEFT comp+IRT gates",
        "purpose": "Compare HEFT, DHEFT, baseline NHEFT, and GHEFT with comp and IRT opening gates enabled simultaneously.",
        "timestamp": timestamp,
