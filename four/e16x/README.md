# E16X: Sixth NCCR Bucket, Balanced CCR/IDR / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e16x/b16x.properties
```
...
```bash
E16X_NUM_SEEDS=1 E16X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e16x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e16x/run_experiment.py
```

Dry run:

```bash
E16X_NUM_SEEDS=1 E16X_LIMIT_RUNS=1 E16X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e16x/run_experiment.py
```
...
```bash
E16X_NUM_SEEDS=20
E16X_MASTER_SEED=151
E16X_TIMEOUT=180
E16X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e16x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e16x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
1.78 < NCCR_total <= 3.16 AND |CCR_data - IDR_image| <= 0.02
```

Additional useful analysis override:

```bash
E16X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e16x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e16x_seed_variant_paired_metrics.csv`
- `e16x_variant_summary.csv`
- `e16x_reference_summary.csv`
- `e16x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e16x` asks the
