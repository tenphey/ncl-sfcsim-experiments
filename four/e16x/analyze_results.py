#!/usr/bin/env python3
"""Analyze E16X NHEFT computation-advantage gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E16X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e16x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E16X comp-advantage result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E16X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E16X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E16X_ABS_TOL", "0.02"))
...
def filter_scenario_rows(ok):
    ...
    scenario_rule = (
        f"1.78 < NCCR_total <= 3.16 AND |CCR_data - IDR_image| <= {BALANCED_ABS_TOL:.4f}"
    )
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E16X NHEFT computation-advantage gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E16X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E16X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e16x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e16x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e16x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e16x_complete_seed_report.csv")
