#!/usr/bin/env python3
"""Analyze E28Z NHEFT comp+DRT gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E28Z
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e28z/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E28Z comp+DRT result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E28Z paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E28Z analysis")
...
MIN_REL_GAP_PCT = float(os.getenv("E28Z_MIN_REL_GAP_PCT", "20.0"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E28Z NHEFT comp+DRT gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E28Z variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E28Z result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e28z_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e28z_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e28z_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e28z_complete_seed_report.csv")
