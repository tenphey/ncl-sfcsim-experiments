# E28Z: Eighth NCCR Bucket, Data-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e28z/b28z.properties
```
...
```bash
E28Z_NUM_SEEDS=1 E28Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e28z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e28z/run_experiment.py
```

Dry run:

```bash
E28Z_NUM_SEEDS=1 E28Z_LIMIT_RUNS=1 E28Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e28z/run_experiment.py
```
...
```bash
E28Z_NUM_SEEDS=20
E28Z_MASTER_SEED=151
E28Z_TIMEOUT=180
E28Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e28z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e28z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E28Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e28z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e28z_seed_variant_paired_metrics.csv`
- `e28z_variant_summary.csv`
- `e28z_reference_summary.csv`
- `e28z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e28z` asks the
