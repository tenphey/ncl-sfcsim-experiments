#!/usr/bin/env python3
"""Analyze E36Z NHEFT IRT-only gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E36Z
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e36z/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E36Z IRT-only result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E36Z paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E36Z analysis")
...
MIN_REL_GAP_PCT = float(os.getenv("E36Z_MIN_REL_GAP_PCT", "20.0"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E36Z NHEFT IRT-only gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E36Z variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E36Z result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e36z_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e36z_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e36z_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e36z_complete_seed_report.csv")
