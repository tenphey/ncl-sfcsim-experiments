# E28Y: Eighth NCCR Bucket, Image-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e28y/b28y.properties
```
...
```bash
E28Y_NUM_SEEDS=1 E28Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e28y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e28y/run_experiment.py
```

Dry run:

```bash
E28Y_NUM_SEEDS=1 E28Y_LIMIT_RUNS=1 E28Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e28y/run_experiment.py
```
...
```bash
E28Y_NUM_SEEDS=20
E28Y_MASTER_SEED=151
E28Y_TIMEOUT=180
E28Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e28y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e28y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E28Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e28y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e28y_seed_variant_paired_metrics.csv`
- `e28y_variant_summary.csv`
- `e28y_reference_summary.csv`
- `e28y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e28y` asks the
