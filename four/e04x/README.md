# E04X: Shared Baseline Scenario Run
...
```text
experiments/four/e04x/b04x.properties
```
...
```bash
E04X_NUM_SEEDS=1 E04X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e04x/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e04x/run_experiment.py
```

Dry run:

```bash
E04X_NUM_SEEDS=1 E04X_LIMIT_RUNS=1 E04X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e04x/run_experiment.py
```

Optional override:

```bash
E04X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e04x` as the common baseline side
