...
Run from the simulator repository root, for example:
  ...
  experiments/four/e12y/run_experiment.py

Useful environment variables:

  E12Y_NUM_SEEDS=5       Run five seeds instead of the default 1000.
  E12Y_MASTER_SEED=151   Change the deterministic seed list.
  E12Y_LIMIT_RUNS=2      Stop after two Java invocations (smoke testing).
  E12Y_DRY_RUN=1         Print commands without launching Java.
  E12Y_TIMEOUT=180       Timeout in seconds for each Java invocation.
  E12Y_BASE_PROPERTIES=/absolute/path/to/file.properties
                       Override the default local b12y.properties scenario file.
"""
...
MASTER_SEED = int(os.getenv("E12Y_MASTER_SEED", "151"))
NUM_SEEDS = int(os.getenv("E12Y_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
LIMIT_RUNS = int(os.getenv("E12Y_LIMIT_RUNS", "0"))
DRY_RUN = os.getenv("E12Y_DRY_RUN", "0") == "1"
TIMEOUT = int(os.getenv("E12Y_TIMEOUT", "180"))
RAW_CSV_NAME = "e12y_results.csv"
...
def resolve_base_properties(project_root):
    override = os.getenv("E12Y_BASE_PROPERTIES")
    if override:
        path = override if os.path.isabs(override) else os.path.abspath(override)
    else:
        path = os.path.join(THIS_DIR, "b12y.properties")
...
def make_seed_list():
    if NUM_SEEDS <= 0:
        raise ValueError("E12Y_NUM_SEEDS must be positive")
    population = range(1000, 1000000)
    if NUM_SEEDS > len(population):
        raise ValueError("E12Y_NUM_SEEDS is larger than the available seed range")
...
def main():
    ...
    manifest = {
        "experiment": "E12Y - NHEFT computation-advantage gate",
        "purpose": "Compare HEFT, DHEFT, baseline NHEFT, and GHEFT with only the computation-advantage gate enabled under the image-dominant second-bucket scenario.",
        "timestamp": timestamp,
    ...
    print("=== E12Y: NHEFT Computation-Advantage Gate ===")
