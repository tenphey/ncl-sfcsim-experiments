# E22X: Second NCCR Bucket, Balanced CCR/IDR / NHEFT Computation + DRT Gate
...
```text
experiments/four/e22x/b22x.properties
```
...
```bash
E22X_NUM_SEEDS=1 E22X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e22x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e22x/run_experiment.py
```

Dry run:

```bash
E22X_NUM_SEEDS=1 E22X_LIMIT_RUNS=1 E22X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e22x/run_experiment.py
```
...
```bash
E22X_NUM_SEEDS=20
E22X_MASTER_SEED=151
E22X_TIMEOUT=180
E22X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e22x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e22x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E22X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e22x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e22x_seed_variant_paired_metrics.csv`
- `e22x_variant_summary.csv`
- `e22x_reference_summary.csv`
- `e22x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e22x` asks the
