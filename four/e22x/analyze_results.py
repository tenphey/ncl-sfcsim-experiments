#!/usr/bin/env python3
"""Analyze E22X NHEFT comp+DRT gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E22X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e22x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E22X comp+DRT result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E22X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E22X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E22X_ABS_TOL", "0.02"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E22X NHEFT comp+DRT gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E22X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E22X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e22x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e22x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e22x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e22x_complete_seed_report.csv")
