#!/usr/bin/env python3
"""Analyze E15Y NHEFT computation-advantage gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E15Y
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e15y/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E15Y comp-advantage result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E15Y paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E15Y analysis")
...
MIN_REL_GAP_PCT = float(os.getenv("E15Y_MIN_REL_GAP_PCT", "20.0"))
...
def filter_scenario_rows(ok):
    ...
    scenario_rule = (
        f"1.00 < NCCR_total <= 1.78 AND CCR_data < IDR_image AND relative CCR/IDR gap >= {MIN_REL_GAP_PCT:.2f}%"
    )
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E15Y NHEFT computation-advantage gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E15Y variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E15Y result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e15y_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e15y_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e15y_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e15y_complete_seed_report.csv")
