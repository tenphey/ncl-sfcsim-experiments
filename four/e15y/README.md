# E15Y: Fifth NCCR Bucket, Image-Dominant / NHEFT Computation-Advantage Gate
...
```text
1.00 < NCCR_total <= 1.78 AND CCR_data < IDR_image
```

The parameter values are reused from:

```text
experiments/three/e45
```

so that the `four/` gate study can be tested under the same tuned fifth-bucket
...
```text
experiments/four/e15y/b15y.properties
```
...
fifth-bucket scenario, while the same seed is reused for both baseline and
comp-gated NHEFT.
...
```bash
E15Y_NUM_SEEDS=1 E15Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e15y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e15y/run_experiment.py
```

Dry run:

```bash
E15Y_NUM_SEEDS=1 E15Y_LIMIT_RUNS=1 E15Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e15y/run_experiment.py
```
...
```bash
E15Y_NUM_SEEDS=20
E15Y_MASTER_SEED=151
E15Y_TIMEOUT=180
E15Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e15y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e15y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
1.00 < NCCR_total <= 1.78 AND CCR_data < IDR_image AND relative CCR/IDR gap >= 20%
```

Additional useful analysis override:

```bash
E15Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e15y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e15y_seed_variant_paired_metrics.csv`
- `e15y_variant_summary.csv`
- `e15y_reference_summary.csv`
- `e15y_complete_seed_report.csv`
...
- under this **one fixed fifth-bucket communication condition**, does the
  computation-advantage gate reduce vCPU usage while keeping more makespan
  benefit than DHEFT?

In other words, `e1` asks the general gate question, while `e15y` asks the
