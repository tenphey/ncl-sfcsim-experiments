#!/usr/bin/env python3
"""Aggregate all E3Y bucket results into cross-bucket overview figures.

This script combines the latest raw results of:

  baseline side: e01y, e02y, e03y, e04y, e05y, e06y, e07y, e08y
  GHEFT side:    e31y, e32y, e33y, e34y, e35y, e36y, e37y, e38y
...
A seed is counted only when:
  ...
  - both rows satisfy the same bucket + y-condition

The output includes one full overview and two four-bucket split figures:

  - e3y_makespan_vcpu_overview.png: all 8 NCCR buckets
  - e3y_makespan_vcpu_overview-1.png: lower four NCCR buckets
  - e3y_makespan_vcpu_overview-2.png: upper four NCCR buckets
...
Usage from the simulator repository root:
  ...
  experiments/.venv/bin/python experiments/four/e3y/analyze_results.py
"""
...
MIN_REL_GAP_PCT = float(os.getenv("E3Y_MIN_REL_GAP_PCT", "20.0"))
...
BUCKET_SPECS = [
    ("(0.10, 0.18]", 0.10, 0.18, "e01y", "e31y"),
    ("(0.18, 0.32]", 0.18, 0.32, "e02y", "e32y"),
    ("(0.32, 0.56]", 0.32, 0.56, "e03y", "e33y"),
    ("(0.56, 1.00]", 0.56, 1.00, "e04y", "e34y"),
    ("(1.00, 1.78]", 1.00, 1.78, "e05y", "e35y"),
    ("(1.78, 3.16]", 1.78, 3.16, "e06y", "e36y"),
    ("(3.16, 5.62]", 3.16, 5.62, "e07y", "e37y"),
    ("(5.62, 10.00]", 5.62, 10.00, "e08y", "e38y"),
]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Aggregate baseline e01y-e08y and GHEFT e31y-e38y into full and split y-condition overview figures."
    )
    parser.add_argument(
        "--output-dir",
        help="Optional output directory. Default: experiments/four/e3y/analysis_<timestamp>_<seed_count>",
    )
...
def validate_filtered_rows(filtered, lower, upper):
    ...
    if not bad_nccr.empty:
        raise RuntimeError(
            ...
            "which should never happen in e3y aggregation"
        )
    ...
    if not bad_gap.empty:
        raise RuntimeError(
            "Filtered rows violate the y-condition relative gap threshold "
...
def save_all_overview_plots(summary_df, output_dir):
    ...
    plot_specs = [
        (
            ordered_df,
            "e3y_makespan_vcpu_overview.png",
            "E3Y overview: baseline vs IRT-only GHEFT across 8 buckets",
        ),
        (
            ordered_df.iloc[:4].reset_index(drop=True),
            "e3y_makespan_vcpu_overview-1.png",
            "E3Y overview: baseline vs IRT-only GHEFT (lower 4 buckets)",
        ),
        (
            ordered_df.iloc[4:8].reset_index(drop=True),
            "e3y_makespan_vcpu_overview-2.png",
            "E3Y overview: baseline vs IRT-only GHEFT (upper 4 buckets)",
        ),
    ]
...
def write_seed_coverage_md(output_dir, summary_df):
    output_path = os.path.join(output_dir, "e3y_seed_coverage_report.md")
    lines = [
        ...
        (
            ...
            f"and the y-condition (`CCR_data < IDR_image` and relative gap >= {MIN_REL_GAP_PCT:.2f}%)."
        ),
        (
...
def write_manifest(output_dir, summary_df):
    payload = {
        "analysis": "E3Y cross-bucket aggregation",
        ...
        "note": (
            "Each bucket uses only paired seeds after y-condition filtering. "
...
def main():
    ...
    summary_csv = os.path.join(output_dir, "e3y_bucket_summary.csv")
    ...
    print("E3Y cross-bucket aggregation complete.")
