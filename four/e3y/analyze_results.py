#!/usr/bin/env python3
"""Aggregate all E3Y bucket results into one cross-bucket overview.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Aggregate baseline e01y-e08y and GHEFT e31y-e38y into one y-condition overview figure."
    )
    parser.add_argument(
        "--output-dir",
        help="Optional output directory. Default: experiments/four/e3y/analysis_<timestamp>",
    )
...
def save_overview_plot(summary_df, output_dir):
    ...
    ax_m.set_title("E3Y overview: baseline vs IRT-only GHEFT across 8 buckets")

    fig.tight_layout()
    output_path = os.path.join(output_dir, "e3y_makespan_vcpu_overview.png")
