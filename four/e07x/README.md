# E07X: Shared Baseline Scenario Run
...
```text
experiments/four/e07x/b07x.properties
```
...
```bash
E07X_NUM_SEEDS=1 E07X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e07x/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e07x/run_experiment.py
```

Dry run:

```bash
E07X_NUM_SEEDS=1 E07X_LIMIT_RUNS=1 E07X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e07x/run_experiment.py
```

Optional override:

```bash
E07X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e07x` as the common baseline side
