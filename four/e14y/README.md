# E14Y: Fourth NCCR Bucket, Image-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e14y/b14y.properties
```
...
```bash
E14Y_NUM_SEEDS=1 E14Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e14y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e14y/run_experiment.py
```

Dry run:

```bash
E14Y_NUM_SEEDS=1 E14Y_LIMIT_RUNS=1 E14Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e14y/run_experiment.py
```
...
```bash
experiments/.venv/bin/python   experiments/four/e14y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e14y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
0.56 < NCCR_total <= 1.00 AND CCR_data < IDR_image AND relative CCR/IDR gap >= 20%
```

Additional useful analysis override:

```bash
E14Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e14y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e14y_seed_variant_paired_metrics.csv`
- `e14y_variant_summary.csv`
- `e14y_reference_summary.csv`
- `e14y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e14y` asks the
