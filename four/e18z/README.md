# E18Z: Eighth NCCR Bucket, Data-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e18z/b18z.properties
```
...
```bash
E18Z_NUM_SEEDS=1 E18Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e18z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e18z/run_experiment.py
```

Dry run:

```bash
E18Z_NUM_SEEDS=1 E18Z_LIMIT_RUNS=1 E18Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e18z/run_experiment.py
```
...
```bash
E18Z_NUM_SEEDS=20
E18Z_MASTER_SEED=151
E18Z_TIMEOUT=180
E18Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e18z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e18z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E18Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e18z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e18z_seed_variant_paired_metrics.csv`
- `e18z_variant_summary.csv`
- `e18z_reference_summary.csv`
- `e18z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e18z` asks the
