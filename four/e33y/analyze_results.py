#!/usr/bin/env python3
"""Analyze E33Y NHEFT IRT-only gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E33Y
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e33y/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E33Y IRT-only result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E33Y paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E33Y analysis")
...
MIN_REL_GAP_PCT = float(os.getenv("E33Y_MIN_REL_GAP_PCT", "20.0"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E33Y NHEFT IRT-only gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E33Y variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E33Y result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e33y_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e33y_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e33y_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e33y_complete_seed_report.csv")
