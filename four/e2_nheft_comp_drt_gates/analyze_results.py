#!/usr/bin/env python3
"""Analyze E2 NHEFT comp+DRT gate results.
...
- comp_drt: NHEFT with comp and DRT opening gates enabled

If a run was stopped in the middle of a seed, that incomplete seed is excluded
from all summaries and plots.  A seed is used only when it has all expected E2
variants.
...
RAW_CSV_NAME = "e2_comp_drt_results.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E2 comp+DRT result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
        help=(
            ...
            "with e2_comp_drt_results.csv is used."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E2 paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E2 analysis")
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E2 NHEFT comp+DRT gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E2 variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E2 result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e2_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e2_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e2_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e2_complete_seed_report.csv")
