#!/usr/bin/env python3
"""Generate paired data for the NHEFT comp+DRT gate experiment.
...
Useful environment variables:
  ...
  E2_NUM_SEEDS=5       Run five seeds instead of the default 1000.
  E2_MASTER_SEED=151   Change the deterministic seed list.
  E2_LIMIT_RUNS=2      Stop after two Java invocations (smoke testing).
  E2_DRY_RUN=1         Print commands without launching Java.
  E2_TIMEOUT=180       Timeout in seconds for each Java invocation.
  E2_BASE_PROPERTIES=/absolute/path/to/file.properties
...
MASTER_SEED = int(os.getenv("E2_MASTER_SEED", "151"))
NUM_SEEDS = int(os.getenv("E2_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
LIMIT_RUNS = int(os.getenv("E2_LIMIT_RUNS", "0"))
DRY_RUN = os.getenv("E2_DRY_RUN", "0") == "1"
TIMEOUT = int(os.getenv("E2_TIMEOUT", "180"))
...
GATE_VARIANT = {
    "label": "comp_drt",
    "display_label": "NHEFT+CompDRT",
    "tolerance": "0.0",
    "comp_advantage": "1",
    "drt_advantage": "1",
    "irt_advantage": "0",
    "description": "Open a new vCPU only when it has earlier EFT and simultaneously has comp and DRT advantages over the best already-used vCPU.",
}
...
def resolve_base_properties(project_root):
    override = os.getenv("E2_BASE_PROPERTIES")
    if override:
...
def make_seed_list():
    if NUM_SEEDS <= 0:
        raise ValueError("E2_NUM_SEEDS must be positive")
    population = range(1000, 1000000)
    if NUM_SEEDS > len(population):
        raise ValueError("E2_NUM_SEEDS is larger than the available seed range")
...
def main():
    ...
    manifest = {
        "experiment": "E2 - NHEFT comp+DRT gates",
        "purpose": "Compare HEFT, DHEFT, baseline NHEFT, and GHEFT with comp and DRT opening gates enabled simultaneously.",
        "timestamp": timestamp,
