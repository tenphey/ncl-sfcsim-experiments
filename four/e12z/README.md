# E12Z: Second NCCR Bucket, Data-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e12z/b12z.properties
```
...
```bash
E12Z_NUM_SEEDS=1 E12Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e12z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e12z/run_experiment.py
```

Dry run:

```bash
E12Z_NUM_SEEDS=1 E12Z_LIMIT_RUNS=1 E12Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e12z/run_experiment.py
```
...
```bash
experiments/.venv/bin/python   experiments/four/e12z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e12z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E12Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e12z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e12z_seed_variant_paired_metrics.csv`
- `e12z_variant_summary.csv`
- `e12z_reference_summary.csv`
- `e12z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e12z` asks the
