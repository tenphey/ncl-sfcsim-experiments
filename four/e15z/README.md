# E15Z: Fifth NCCR Bucket, Data-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e15z/b15z.properties
```
...
```bash
E15Z_NUM_SEEDS=1 E15Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e15z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e15z/run_experiment.py
```

Dry run:

```bash
E15Z_NUM_SEEDS=1 E15Z_LIMIT_RUNS=1 E15Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e15z/run_experiment.py
```
...
```bash
E15Z_NUM_SEEDS=20
E15Z_MASTER_SEED=151
E15Z_TIMEOUT=180
E15Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e15z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e15z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E15Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e15z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e15z_seed_variant_paired_metrics.csv`
- `e15z_variant_summary.csv`
- `e15z_reference_summary.csv`
- `e15z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e15z` asks the
