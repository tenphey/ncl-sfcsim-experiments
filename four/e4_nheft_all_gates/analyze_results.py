#!/usr/bin/env python3
"""Analyze E4 NHEFT all-gates results.
...
- all_gates: NHEFT with comp, DRT, and IRT opening gates all enabled

If a run was stopped in the middle of a seed, that incomplete seed is excluded
from all summaries and plots.  A seed is used only when it has all expected E4
variants.
...
RAW_CSV_NAME = "e4_all_gates_results.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E4 all-gates result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
        help=(
            ...
            "with e4_all_gates_results.csv is used."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E4 paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E4 analysis")
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E4 NHEFT all-gates analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E4 variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E4 result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e4_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e4_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e4_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e4_complete_seed_report.csv")
