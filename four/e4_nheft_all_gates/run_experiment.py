#!/usr/bin/env python3
"""Generate paired data for the NHEFT all-gates experiment.
...
Useful environment variables:
  ...
  E4_NUM_SEEDS=5       Run five seeds instead of the default 1000.
  E4_MASTER_SEED=151   Change the deterministic seed list.
  E4_LIMIT_RUNS=2      Stop after two Java invocations (smoke testing).
  E4_DRY_RUN=1         Print commands without launching Java.
  E4_TIMEOUT=180       Timeout in seconds for each Java invocation.
  E4_BASE_PROPERTIES=/absolute/path/to/file.properties
...
MASTER_SEED = int(os.getenv("E4_MASTER_SEED", "151"))
NUM_SEEDS = int(os.getenv("E4_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
LIMIT_RUNS = int(os.getenv("E4_LIMIT_RUNS", "0"))
DRY_RUN = os.getenv("E4_DRY_RUN", "0") == "1"
TIMEOUT = int(os.getenv("E4_TIMEOUT", "180"))
...
GATE_VARIANT = {
    "label": "all_gates",
    "display_label": "NHEFT+AllGates",
    "tolerance": "0.0",
    "comp_advantage": "1",
    "drt_advantage": "1",
    "irt_advantage": "1",
    "description": "Open a new vCPU only when it has earlier EFT and simultaneously has comp, DRT, and IRT advantages over the best already-used vCPU.",
}
...
def resolve_base_properties(project_root):
    override = os.getenv("E4_BASE_PROPERTIES")
    if override:
...
def make_seed_list():
    if NUM_SEEDS <= 0:
        raise ValueError("E4_NUM_SEEDS must be positive")
    population = range(1000, 1000000)
    if NUM_SEEDS > len(population):
        raise ValueError("E4_NUM_SEEDS is larger than the available seed range")
...
def main():
    ...
    manifest = {
        "experiment": "E4 - NHEFT all opening gates",
        "purpose": "Compare HEFT, DHEFT, baseline NHEFT, and GHEFT with comp, DRT, and IRT opening gates enabled simultaneously.",
        "timestamp": timestamp,
    ...
    print("=== E4: NHEFT All Opening Gates ===")
