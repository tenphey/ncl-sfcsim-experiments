# E06X: Shared Baseline Scenario Run
...
```text
experiments/four/e06x/b06x.properties
```
...
```bash
E06X_NUM_SEEDS=1 E06X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e06x/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e06x/run_experiment.py
```

Dry run:

```bash
E06X_NUM_SEEDS=1 E06X_LIMIT_RUNS=1 E06X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e06x/run_experiment.py
```

Optional override:

```bash
E06X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e06x` as the common baseline side
