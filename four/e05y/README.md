# E05Y: Shared Baseline Scenario Run
...
```text
experiments/four/e05y/b05y.properties
```
...
```bash
E05Y_NUM_SEEDS=1 E05Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e05y/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e05y/run_experiment.py
```

Dry run:

```bash
E05Y_NUM_SEEDS=1 E05Y_LIMIT_RUNS=1 E05Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e05y/run_experiment.py
```

Optional override:

```bash
E05Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e05y` as the common baseline side
