#!/usr/bin/env python3
"""Analyze E48X NHEFT DRT-or-IRT gate results.
...
- drt_or_irt: NHEFT with the DRT-or-IRT gate enabled

If a run was stopped in the middle of a seed, that incomplete seed is excluded
from all summaries and plots.  A seed is used only when it has all expected E48X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e48x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E48X DRT-or-IRT result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E48X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E48X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E48X_ABS_TOL", "0.02"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E48X NHEFT DRT-or-IRT gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E48X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E48X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e48x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e48x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e48x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e48x_complete_seed_report.csv")
