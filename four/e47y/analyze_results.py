#!/usr/bin/env python3
"""Analyze E47Y NHEFT DRT-or-IRT gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E47Y
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e47y/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E47Y DRT-or-IRT result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E47Y paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E47Y analysis")
...
MIN_REL_GAP_PCT = float(os.getenv("E47Y_MIN_REL_GAP_PCT", "20.0"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E47Y NHEFT DRT-or-IRT gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E47Y variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E47Y result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e47y_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e47y_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e47y_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e47y_complete_seed_report.csv")
