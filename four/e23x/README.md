# E23X: Third NCCR Bucket, Balanced CCR/IDR / NHEFT Computation + DRT Gate
...
```text
experiments/four/e23x/b23x.properties
```
...
```bash
E23X_NUM_SEEDS=1 E23X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e23x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e23x/run_experiment.py
```

Dry run:

```bash
E23X_NUM_SEEDS=1 E23X_LIMIT_RUNS=1 E23X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e23x/run_experiment.py
```
...
```bash
E23X_NUM_SEEDS=20
E23X_MASTER_SEED=151
E23X_TIMEOUT=180
E23X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e23x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e23x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E23X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e23x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e23x_seed_variant_paired_metrics.csv`
- `e23x_variant_summary.csv`
- `e23x_reference_summary.csv`
- `e23x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e23x` asks the
