# E21Z: First NCCR Bucket, Data-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e21z/b21z.properties
```
...
```bash
E21Z_NUM_SEEDS=1 E21Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e21z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e21z/run_experiment.py
```

Dry run:

```bash
E21Z_NUM_SEEDS=1 E21Z_LIMIT_RUNS=1 E21Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e21z/run_experiment.py
```
...
```bash
E21Z_NUM_SEEDS=20
E21Z_MASTER_SEED=151
E21Z_TIMEOUT=180
E21Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e21z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e21z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E21Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e21z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e21z_seed_variant_paired_metrics.csv`
- `e21z_variant_summary.csv`
- `e21z_reference_summary.csv`
- `e21z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e21z` asks the
