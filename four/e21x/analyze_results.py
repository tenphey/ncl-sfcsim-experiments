#!/usr/bin/env python3
"""Analyze E21X NHEFT DRT-only gate results.
...
- drt_only: NHEFT with only the DRT gate enabled

If a run was stopped in the middle of a seed, that incomplete seed is excluded
from all summaries and plots.  A seed is used only when it has all expected E21X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e21x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E21X DRT-only result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E21X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E21X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E21X_ABS_TOL", "0.02"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E21X NHEFT DRT-only gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E21X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E21X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e21x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e21x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e21x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e21x_complete_seed_report.csv")
