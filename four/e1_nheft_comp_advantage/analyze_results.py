#!/usr/bin/env python3
"""Analyze E1 NHEFT computation-advantage gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E1
variants.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E1 comp-advantage result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
        help=(
            ...
            "with e1_comp_advantage_results.csv is used."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E1 paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E1 analysis")
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E1 NHEFT computation-advantage gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E1 variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E1 result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e1_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e1_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e1_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e1_complete_seed_report.csv")
