# E25Z: Fifth NCCR Bucket, Data-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e25z/b25z.properties
```
...
```bash
E25Z_NUM_SEEDS=1 E25Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e25z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e25z/run_experiment.py
```

Dry run:

```bash
E25Z_NUM_SEEDS=1 E25Z_LIMIT_RUNS=1 E25Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e25z/run_experiment.py
```
...
```bash
E25Z_NUM_SEEDS=20
E25Z_MASTER_SEED=151
E25Z_TIMEOUT=180
E25Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e25z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e25z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E25Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e25z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e25z_seed_variant_paired_metrics.csv`
- `e25z_variant_summary.csv`
- `e25z_reference_summary.csv`
- `e25z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e25z` asks the
