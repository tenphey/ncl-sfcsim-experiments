# E23Z: Third NCCR Bucket, Data-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e23z/b23z.properties
```
...
```bash
E23Z_NUM_SEEDS=1 E23Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e23z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e23z/run_experiment.py
```

Dry run:

```bash
E23Z_NUM_SEEDS=1 E23Z_LIMIT_RUNS=1 E23Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e23z/run_experiment.py
```
...
```bash
E23Z_NUM_SEEDS=20
E23Z_MASTER_SEED=151
E23Z_TIMEOUT=180
E23Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e23z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e23z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E23Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e23z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e23z_seed_variant_paired_metrics.csv`
- `e23z_variant_summary.csv`
- `e23z_reference_summary.csv`
- `e23z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e23z` asks the
