# E26Z: Sixth NCCR Bucket, Data-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e26z/b26z.properties
```
...
```bash
E26Z_NUM_SEEDS=1 E26Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e26z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e26z/run_experiment.py
```

Dry run:

```bash
E26Z_NUM_SEEDS=1 E26Z_LIMIT_RUNS=1 E26Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e26z/run_experiment.py
```
...
```bash
E26Z_NUM_SEEDS=20
E26Z_MASTER_SEED=151
E26Z_TIMEOUT=180
E26Z_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e26z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e26z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E26Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e26z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e26z_seed_variant_paired_metrics.csv`
- `e26z_variant_summary.csv`
- `e26z_reference_summary.csv`
- `e26z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e26z` asks the
