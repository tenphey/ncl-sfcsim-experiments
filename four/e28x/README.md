# E28X: Eighth NCCR Bucket, Balanced CCR/IDR / NHEFT Computation + DRT Gate
...
```text
experiments/four/e28x/b28x.properties
```
...
```bash
E28X_NUM_SEEDS=1 E28X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e28x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e28x/run_experiment.py
```

Dry run:

```bash
E28X_NUM_SEEDS=1 E28X_LIMIT_RUNS=1 E28X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e28x/run_experiment.py
```
...
```bash
E28X_NUM_SEEDS=20
E28X_MASTER_SEED=151
E28X_TIMEOUT=180
E28X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e28x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e28x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E28X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e28x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e28x_seed_variant_paired_metrics.csv`
- `e28x_variant_summary.csv`
- `e28x_reference_summary.csv`
- `e28x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e28x` asks the
