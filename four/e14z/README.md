# E14Z: Fourth NCCR Bucket, Data-Dominant / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e14z/b14z.properties
```
...
```bash
E14Z_NUM_SEEDS=1 E14Z_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e14z/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e14z/run_experiment.py
```

Dry run:

```bash
E14Z_NUM_SEEDS=1 E14Z_LIMIT_RUNS=1 E14Z_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e14z/run_experiment.py
```
...
```bash
experiments/.venv/bin/python   experiments/four/e14z/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e14z/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E14Z_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e14z_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e14z_seed_variant_paired_metrics.csv`
- `e14z_variant_summary.csv`
- `e14z_reference_summary.csv`
- `e14z_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e14z` asks the
