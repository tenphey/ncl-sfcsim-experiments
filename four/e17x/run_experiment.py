...
Run from the simulator repository root, for example:
  ...
  experiments/four/e17x/run_experiment.py

Useful environment variables:

  E17X_NUM_SEEDS=5       Run five seeds instead of the default 1000.
  E17X_MASTER_SEED=151   Change the deterministic seed list.
  E17X_LIMIT_RUNS=2      Stop after two Java invocations (smoke testing).
  E17X_DRY_RUN=1         Print commands without launching Java.
  E17X_TIMEOUT=180       Timeout in seconds for each Java invocation.
  E17X_BASE_PROPERTIES=/absolute/path/to/file.properties
                       Override the default local b17x.properties scenario file.
"""
...
MASTER_SEED = int(os.getenv("E17X_MASTER_SEED", "151"))
NUM_SEEDS = int(os.getenv("E17X_NUM_SEEDS", str(NUM_SEEDS_DEFAULT)))
LIMIT_RUNS = int(os.getenv("E17X_LIMIT_RUNS", "0"))
DRY_RUN = os.getenv("E17X_DRY_RUN", "0") == "1"
TIMEOUT = int(os.getenv("E17X_TIMEOUT", "180"))
...
def resolve_base_properties(project_root):
    override = os.getenv("E17X_BASE_PROPERTIES")
    if override:
        path = override if os.path.isabs(override) else os.path.abspath(override)
    else:
        path = os.path.join(THIS_DIR, "b17x.properties")
...
def make_seed_list():
    if NUM_SEEDS <= 0:
        raise ValueError("E17X_NUM_SEEDS must be positive")
    population = range(1000, 1000000)
    if NUM_SEEDS > len(population):
        raise ValueError("E17X_NUM_SEEDS is larger than the available seed range")
...
def main():
    ...
    manifest = {
        "experiment": "E17X - NHEFT computation-advantage gate",
    ...
    print("=== E17X: NHEFT Computation-Advantage Gate ===")
