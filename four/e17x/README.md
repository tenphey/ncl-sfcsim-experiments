# E17X: Seventh NCCR Bucket, Balanced CCR/IDR / NHEFT Computation-Advantage Gate
...
```text
experiments/four/e17x/b17x.properties
```
...
```bash
E17X_NUM_SEEDS=1 E17X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e17x/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e17x/run_experiment.py
```

Dry run:

```bash
E17X_NUM_SEEDS=1 E17X_LIMIT_RUNS=1 E17X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e17x/run_experiment.py
```
...
```bash
experiments/.venv/bin/python   experiments/four/e17x/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e17x/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E17X_ABS_TOL=0.02   Balanced CCR/IDR absolute tolerance during analysis.
```
...
- `e17x_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e17x_seed_variant_paired_metrics.csv`
- `e17x_variant_summary.csv`
- `e17x_reference_summary.csv`
- `e17x_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e17x` asks the
