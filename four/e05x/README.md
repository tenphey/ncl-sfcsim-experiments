# E05X: Shared Baseline Scenario Run
...
```text
experiments/four/e05x/b05x.properties
```
...
```bash
E05X_NUM_SEEDS=1 E05X_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e05x/run_experiment.py
```

Full default run:

```bash
experiments/.venv/bin/python   experiments/four/e05x/run_experiment.py
```

Dry run:

```bash
E05X_NUM_SEEDS=1 E05X_LIMIT_RUNS=1 E05X_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e05x/run_experiment.py
```

Optional override:

```bash
E05X_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
- `e05x` as the common baseline side
