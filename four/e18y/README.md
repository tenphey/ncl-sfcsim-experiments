# E18Y: Eighth NCCR Bucket, Image-Dominant / NHEFT Computation-Advantage Gate
...
```text
5.62 < NCCR_total <= 10.00 AND CCR_data < IDR_image
```

The parameter values are reused from:

```text
experiments/three/e48
```

so that the `four/` gate study can be tested under the same tuned eighth-bucket
...
```text
experiments/four/e18y/b18y.properties
```
...
eighth-bucket scenario, while the same seed is reused for both baseline and
comp-gated NHEFT.
...
```bash
E18Y_NUM_SEEDS=1 E18Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e18y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e18y/run_experiment.py
```

Dry run:

```bash
E18Y_NUM_SEEDS=1 E18Y_LIMIT_RUNS=1 E18Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e18y/run_experiment.py
```
...
```bash
E18Y_NUM_SEEDS=20
E18Y_MASTER_SEED=151
E18Y_TIMEOUT=180
E18Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e18y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e18y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```text
5.62 < NCCR_total <= 10.00 AND CCR_data < IDR_image AND relative CCR/IDR gap >= 20%
```

Additional useful analysis override:

```bash
E18Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e18y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e18y_seed_variant_paired_metrics.csv`
- `e18y_variant_summary.csv`
- `e18y_reference_summary.csv`
- `e18y_complete_seed_report.csv`
...
- under this **one fixed eighth-bucket communication condition**, does the
  computation-advantage gate reduce vCPU usage while keeping more makespan
  benefit than DHEFT?

In other words, `e1` asks the general gate question, while `e18y` asks the
