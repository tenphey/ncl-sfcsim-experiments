#!/usr/bin/env python3
"""Analyze E47X NHEFT DRT-or-IRT gate results.
...
from all summaries and plots.  A seed is used only when it has all expected E47X
variants.

Usage from the simulator repository root:

  experiments/.venv/bin/python \
      experiments/four/e47x/analyze_results.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an existing E47X DRT-or-IRT result folder."
    )
    parser.add_argument(
        "result_folder",
        nargs="?",
...
def expected_variants_from_manifest(manifest, df):
    ...
    if BASE_VARIANT not in labels:
        raise RuntimeError("The baseline variant is required for E47X paired analysis")
    if len(labels) < 2:
        raise RuntimeError("At least two variants are required for E47X analysis")
...
BALANCED_ABS_TOL = float(os.getenv("E47X_ABS_TOL", "0.02"))
...
def write_analysis_manifest(
    ...
    manifest = {
        "analysis": "E47X NHEFT DRT-or-IRT gate analysis",
        ...
        "complete_seed_rule": (
            "Only seeds with all expected E47X variants are included in summaries "
            "and plots."
        ),
...
def main():
    ...
    if not result_dir:
        print("No E47X result folder found.")
    ...
    paired_csv = os.path.join(output_dir, "e47x_seed_variant_paired_metrics.csv")
    summary_csv = os.path.join(output_dir, "e47x_variant_summary.csv")
    reference_csv = os.path.join(output_dir, "e47x_reference_summary.csv")
    complete_seed_report_csv = os.path.join(output_dir, "e47x_complete_seed_report.csv")
