# E22Y: Second NCCR Bucket, Image-Dominant / NHEFT Computation + DRT Gate
...
```text
experiments/four/e22y/b22y.properties
```
...
```bash
E22Y_NUM_SEEDS=1 E22Y_LIMIT_RUNS=1   experiments/.venv/bin/python   experiments/four/e22y/run_experiment.py
```

Full default experiment:

```bash
experiments/.venv/bin/python   experiments/four/e22y/run_experiment.py
```

Dry run:

```bash
E22Y_NUM_SEEDS=1 E22Y_LIMIT_RUNS=1 E22Y_DRY_RUN=1   experiments/.venv/bin/python   experiments/four/e22y/run_experiment.py
```
...
```bash
E22Y_NUM_SEEDS=20
E22Y_MASTER_SEED=151
E22Y_TIMEOUT=180
E22Y_BASE_PROPERTIES=/absolute/path/to/file.properties
```
...
```bash
experiments/.venv/bin/python   experiments/four/e22y/analyze_results.py
```

Analyze a specific result folder:

```bash
experiments/.venv/bin/python   experiments/four/e22y/analyze_results.py   run_YYYYMMDD_HHMMSS_151_1000
```
...
```bash
E22Y_MIN_REL_GAP_PCT=20   Minimum relative CCR/IDR gap during analysis.
```
...
- `e22y_results.csv`: long-format raw metrics, one row per `seed x variant`
...
- `e22y_seed_variant_paired_metrics.csv`
- `e22y_variant_summary.csv`
- `e22y_reference_summary.csv`
- `e22y_complete_seed_report.csv`
...
In other words, `e1` asks the general gate question, while `e22y` asks the
