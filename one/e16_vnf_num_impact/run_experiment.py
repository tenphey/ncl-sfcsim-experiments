#!/usr/bin/env python3
"""
E16 Experiment: VNF Quantity / Scale Impact

Goal:
- Keep the SFC count fixed.
- Sweep the total VNF count by scaling the per-SFC VNF count.
- Keep download-pressure settings strong so changes in scheduling scale are visible.

Hypothesis:
- As total VNF count grows, download overlap and critical-path propagation become
  more pronounced, which should make NHEFT's bandwidth-slot model easier to observe.

Usage:
  python3 run_experiment.py
  E16_DRY_RUN=1 python3 run_experiment.py
  E16_NUM_SEEDS=10 python3 run_experiment.py
"""
...
# Sweep total VNF count, not image size or type diversity.
EFFECTIVE_TOTAL_VNF_LEVELS = env_int_list(
    "E16_TOTAL_VNF_LEVELS",
    default=[80, 160, 240, 320],
)

# Fixed knobs for a download-heavy setting.
MULTIPLE_SFC_NUM = env_int("E16_MULTIPLE_SFC_NUM", default=4)
VNF_TYPE_MIN = env_int("E16_VNF_TYPE_MIN", default=1)
VNF_TYPE_MAX = env_int("E16_VNF_TYPE_MAX", default=12)
VNF_IMAGE_SIZE_MIN = env_int("E16_IMAGE_MIN", default=6000)
VNF_IMAGE_SIZE_MAX = env_int("E16_IMAGE_MAX", default=12000)
REPOSITORY_BW = env_int("E16_REPOSITORY_BW", default=200)
DATACENTER_EXTERNALBW_MIN = env_int("E16_DATACENTER_EXTERNALBW_MIN", default=500)
DATACENTER_EXTERNALBW_MAX = env_int("E16_DATACENTER_EXTERNALBW_MAX", default=1000)
HOST_BW_MIN = env_int("E16_HOST_BW_MIN", default=1000)
HOST_BW_MAX = env_int("E16_HOST_BW_MAX", default=2000)

DRY_RUN = env_int("E16_DRY_RUN", default=0) == 1
LIMIT_RUNS = env_int("E16_LIMIT_RUNS", default=0)

# Seeding
MASTER_SEED = env_int("E16_MASTER_SEED", default=150)
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = env_int("E16_NUM_SEEDS", default=NUM_SEEDS_DEFAULT)
...
def main():
    ...
    csv_path = os.path.join(out_dir, "grid_e16_results.csv")
    results = []
    base = read_props(BASE_PROPS)

    total_runs = len(EFFECTIVE_TOTAL_VNF_LEVELS) * len(SEEDS)
    run_count = 0

    print("=== E16 Experiment: VNF Quantity / Scale Impact ===")
    print(f"Master seed: {MASTER_SEED}")
    print(f"Number of seeds: {NUM_SEEDS}")
    print(f"Effective total VNF levels: {EFFECTIVE_TOTAL_VNF_LEVELS}")
    print(f"Fixed multiple_sfc_num: {MULTIPLE_SFC_NUM}")
    print(f"VNF type range: {VNF_TYPE_MIN}-{VNF_TYPE_MAX}")
    print(f"Image size range: {VNF_IMAGE_SIZE_MIN}-{VNF_IMAGE_SIZE_MAX}")
    print(f"Repository BW: {REPOSITORY_BW}")
    ...
    for total_vnf_num in EFFECTIVE_TOTAL_VNF_LEVELS:
        if total_vnf_num % MULTIPLE_SFC_NUM != 0:
            raise ValueError(
                f"total_vnf_num={total_vnf_num} is not divisible by multiple_sfc_num={MULTIPLE_SFC_NUM}"
            )
        per_sfc_vnf_num = total_vnf_num // MULTIPLE_SFC_NUM
        ...
        for s in SEEDS:
            ...
            props["sfc_vnf_num"] = str(total_vnf_num)
            ...
            print(
                ...
                f"total_vnf_num={total_vnf_num} per_sfc={per_sfc_vnf_num} seed={s}",
                end="",
            )
            ...
            if DRY_RUN:
                ...
                try:
                    ...
                    log_name = os.path.join(
                        logs_dir,
                        f"run_seed_{s}_total_{total_vnf_num}_msfc_{MULTIPLE_SFC_NUM}.log",
                    )
                    try:
            ...
            results.append(
                {
                    "effective_total_vnf_num": total_vnf_num,
    ...
    manifest = {
        "experiment": "E16 - VNF Quantity / Scale Impact",
        "timestamp": timestamp,
        "master_seed": MASTER_SEED,
        "num_seeds": NUM_SEEDS,
        "seeds_used": SEEDS,
        "effective_total_vnf_levels": EFFECTIVE_TOTAL_VNF_LEVELS,
        "fixed_multiple_sfc_num": MULTIPLE_SFC_NUM,
