#!/usr/bin/env python3
"""
E12 Experiment: VNF Type Diversity Impact

Scans vnf_type_max ∈ {4, 8, 12, 20} while keeping all other parameters
the same as base.properties.
Generates grid_e12_results.csv for downstream analysis.

Usage:
  python3 run_experiment.py                    # Normal run
  E12_DRY_RUN=1 python3 run_experiment.py      # Dry run (print commands only)
  E12_NUM_SEEDS=10 python3 run_experiment.py   # Use 10 seeds instead of default
"""
...
VNF_TYPE_MAXS = [5, 10, 15, 20]
DRY_RUN = env_int('E12_DRY_RUN', default=0) == 1
LIMIT_RUNS = env_int('E12_LIMIT_RUNS', default=0)

# Seeding
MASTER_SEED = env_int('E12_MASTER_SEED', default=150)
NUM_SEEDS_DEFAULT = 100
NUM_SEEDS = env_int('E12_NUM_SEEDS', default=NUM_SEEDS_DEFAULT)
...
def main():
    ...
    csv_path = os.path.join(out_dir, 'grid_e12_results.csv')
    results = []
    ...
    print(f'=== E12 Experiment: VNF Type Diversity Impact ===')
    print(f'Master seed: {MASTER_SEED}')
    print(f'Number of seeds: {NUM_SEEDS}')
    print(f'VNF type max values: {VNF_TYPE_MAXS}')
    ...
    for vtm in VNF_TYPE_MAXS:
        ...
        for s in SEEDS:
            ...
            props['vnf_type_min'] = str(vtm_min)
            props['random_seed'] = str(s)
            props['random_seed'] = str(s)

            props['datacenter_externalbw_min'] = str(500)
            props['datacenter_externalbw_max'] = str(1000)
            props['host_bw_min'] = str(1000)
            props['host_bw_max'] = str(2000)
            props['repository_bw'] = str(1000)
            ...
            print(f'[{run_count}/{total_runs}] vnf_type_min={vtm_min} vnf_type_max={vtm} seed={s}', end='')
            ...
            if DRY_RUN:
                ...
                try:
                    ...
                    log_name = os.path.join(logs_dir, f'run_seed_{s}_vtm_{vtm}.log')
                    try:
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['vnf_type_min', 'vnf_type_max', 'seed', 'HEFT', 'DHEFT', 'NHEFT', 'time_sec'])
        writer.writeheader()
    ...
    # Write manifest
    manifest = {
        'experiment': 'E12 - VNF Type Diversity Impact',
        'timestamp': timestamp,
        'master_seed': MASTER_SEED,
        'num_seeds': NUM_SEEDS,
        'seeds_used': SEEDS,
        'vnf_type_mins': [x - 4 for x in VNF_TYPE_MAXS],
