#!/usr/bin/env python3
"""
Analyze E12 results: generate a grouped bar chart showing DHEFT vs NHEFT
average makespan by vnf_type_max.
...
def resolve_log_path(result_dir, seed, vnf_type_max):
    """Resolve per-run log path under logs/."""
    logs_dir = os.path.join(result_dir, "logs")
    fname = f"run_seed_{int(seed)}_vtm_{int(vnf_type_max)}.log"
...
def main():
    ...
    csv_path = os.path.join(result_dir, 'grid_e12_results.csv')
    ...
    summary_csv = os.path.join(result_dir, 'grid_e12_summary.csv')
    ...
    for vtm in sorted(df['vnf_type_max'].unique()):
        ...
        for _, row in loss_group.iterrows():
            ...
            log_path = resolve_log_path(result_dir, row['seed'], row['vnf_type_max'])
    ...
    loss_json_path = os.path.join(result_dir, 'e12_nheft_loss_seeds_by_vnf_type_max.json')
    with open(loss_json_path, 'w') as f:
        json.dump(loss_details_by_vtm, f, indent=2)

    print(f'\nSummary CSV: {summary_csv}')
    print(f'Loss-seeds JSON saved: {loss_json_path}')
    print('\n=== E12 Results by VNF Type Max ===')
    console_df = summary_df[['vnf_type_max', 'DHEFT_mean', 'NHEFT_mean', 'gain_N_over_D_median', 'gain_N_over_D_mean', 'wins_count', 'p_value', 'n_runs']].copy()
    ...
    table_path = os.path.join(result_dir, 'e12_summary_table.png')
    draw_table_image(display_df, 'E12 Results by VNF Type Max', table_path)
    print(f'Summary table image saved: {table_path}')

    # Grouped bar chart: mean makespan by vnf_type_max
    fig, ax = plt.subplots(figsize=(12, 7))

    levels = summary_df['vnf_type_max'].values
    ...
    ax.set_xlabel('VNF Type Max', fontsize=13, fontweight='bold')
    ax.set_ylabel('Average Makespan (seconds)', fontsize=13, fontweight='bold')
    ax.set_title('E12 Experiment: DHEFT vs NHEFT Makespan by VNF Type Max', fontsize=14, fontweight='bold')
    ax.set_xticks(x_pos)
    ax.set_xticklabels([str(int(v)) for v in levels], fontsize=12)
    ...
    plt.tight_layout()
    chart_path = os.path.join(result_dir, 'e12_makespan_comparison.png')
    plt.savefig(chart_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f'Grouped bar chart saved: {chart_path}')
