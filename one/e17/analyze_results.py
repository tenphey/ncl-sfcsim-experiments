#!/usr/bin/env python3
"""
Analyze E17 results: repository bandwidth x multiple_sfc_num.

Outputs:
- grid_e17_summary.csv
- e17_summary_table.png
...
Usage:
  ...
  python3 analyze_results.py run_20260530_180000_150_100
"""
...
CACHE_ROOT = os.path.join(tempfile.gettempdir(), "e17_mpl_cache")
...
def main():
    ...
    csv_path = os.path.join(result_dir, "grid_e17_results.csv")
    ...
    grouped = df.groupby(["repository_bw", "multiple_sfc_num"])
    summary_rows = []

    for (repo_bw, msfc), group in grouped:
        n_runs = len(group)
    ...
    summary_df = pd.DataFrame(summary_rows).sort_values(["repository_bw", "multiple_sfc_num"])
    summary_csv = os.path.join(result_dir, "grid_e17_summary.csv")
    ...
    loss_json_path = os.path.join(result_dir, "e17_nheft_loss_seeds_by_repository_bw_multiple_sfc_num.json")
    with open(loss_json_path, "w") as f:
        json.dump(loss_details_by_repo_msfc, f, indent=2)

    print(f"\nSummary CSV: {summary_csv}")
    print(f"Loss-seeds JSON saved: {loss_json_path}")
    print("\n=== E17 Results by repository_bw x multiple_sfc_num ===")
    ...
    display_df["repository_bw"] = display_df["repository_bw"].map(lambda x: f"{int(x)}")
    display_df["multiple_sfc_num"] = display_df["multiple_sfc_num"].map(lambda x: f"{int(x)}")
    for col in ["DHEFT_mean", "NHEFT_mean", "gain_N_over_D_median", "gain_N_over_D_mean", "win_rate"]:
    ...
    table_path = os.path.join(result_dir, "e17_summary_table.png")
    draw_table_image(display_df, "E17 Results by repository_bw x multiple_sfc_num", table_path)
    ...
    gain_heatmap = os.path.join(result_dir, "e17_gain_heatmap.png")
    draw_heatmap(
        gain_matrix,
        "E17 Experiment: NHEFT Gain % over DHEFT",
        "Gain %",
        gain_heatmap,
        fmt="{:.2f}",
        cmap="YlGn",
    ...
    winrate_heatmap = os.path.join(result_dir, "e17_winrate_heatmap.png")
    draw_heatmap(
        winrate_matrix,
        "E17 Experiment: NHEFT Win Rate",
        "Win Rate",
        winrate_heatmap,
        fmt="{:.2f}",
        cmap="Blues",
    ...
    # Trend plot: repository_bw on x-axis, one line per multiple_sfc_num
    ...
    ax.set_title(
        "E17 Experiment: Repository BW x Concurrent Download Opportunity",
        fontsize=14,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    trend_path = os.path.join(result_dir, "e17_repo_bw_trend.png")
