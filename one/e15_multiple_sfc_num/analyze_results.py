#!/usr/bin/env python3
"""
Analyze E15 results: DHEFT vs NHEFT under different multiple_sfc_num levels.

Usage:
  python3 analyze_results.py <result_folder>
  python3 analyze_results.py run_159_20260530_123000
"""
...
def resolve_log_path(result_dir, seed, multiple_sfc_num, per_sfc_vnf_num):
    ...
    new_name = os.path.join(logs_dir, f"run_seed_{seed_i}_msfc_{msfc_i}_per_{per_i}.log")
    if os.path.exists(new_name):
        return new_name

    # Legacy pattern contains master seed prefix.
    legacy_pattern = os.path.join(logs_dir, f"run_*_seed_{seed_i}_msfc_{msfc_i}_per_{per_i}.log")
...
def main():
    ...
    csv_path = os.path.join(result_dir, 'grid_e15_results.csv')
    ...
    grouped = df.groupby('multiple_sfc_num')
    summary_rows = []

    for msfc, group in grouped:
        n_runs = len(group)
    ...
    summary_df = pd.DataFrame(summary_rows).sort_values('multiple_sfc_num')
    summary_csv = os.path.join(result_dir, 'grid_e15_summary.csv')
    summary_df.to_csv(summary_csv, index=False)

    # Build a multiple_sfc_num -> {seed -> execution details} JSON where NHEFT loses to DHEFT.
    if 'seed' not in df.columns:
        raise SystemExit("Missing required column: seed")
    loss_details_by_msfc = {}
    for msfc in sorted(df['multiple_sfc_num'].unique()):
        group = df[df['multiple_sfc_num'] == msfc]
        ...
        for _, row in loss_group.iterrows():
            ...
            if 'per_sfc_vnf_num' in row.index and not pd.isna(row['per_sfc_vnf_num']):
                ...
                per_sfc_vnf_num = 0
            log_path = resolve_log_path(result_dir, row['seed'], row['multiple_sfc_num'], per_sfc_vnf_num)
            ...
            seed_map[seed_key] = {
                ...
                'multiple_sfc_num': int(row['multiple_sfc_num']),
        ...
        loss_details_by_msfc[str(int(msfc))] = seed_map

    loss_json_path = os.path.join(result_dir, 'e15_nheft_loss_seeds_by_multiple_sfc_num.json')
    with open(loss_json_path, 'w') as f:
        json.dump(loss_details_by_msfc, f, indent=2)

    print(f'\nSummary CSV: {summary_csv}')
    print(f'Loss-seeds JSON saved: {loss_json_path}')
    print('\n=== E15 Results by multiple_sfc_num ===')
    console_df = summary_df[['multiple_sfc_num', 'DHEFT_mean', 'NHEFT_mean', 'gain_N_over_D_median', 'gain_N_over_D_mean', 'wins_count', 'p_value', 'n_runs']].copy()
    ...
    display_df['multiple_sfc_num'] = display_df['multiple_sfc_num'].map(lambda x: f"{int(x)}")
    ...
    table_path = os.path.join(result_dir, 'e15_summary_table.png')
    draw_table_image(display_df, 'E15 Results by multiple_sfc_num', table_path)
    print(f'Summary table image saved: {table_path}')

    # Plot 1: Gain trend
    fig, ax = plt.subplots(figsize=(12, 7))

    x = summary_df['multiple_sfc_num'].values
    ...
    ax.plot(x, gains_mean, marker='^', markersize=8, linewidth=1.5,
            label='NHEFT gain % (arithmetic mean)', color='#F18F01', linestyle=':')
    ...
    ax.set_title('E15 Experiment: NHEFT Gain vs Concurrency Opportunity', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    trend_path = os.path.join(result_dir, 'e15_gain_trend.png')
    ...
    labels = summary_df['multiple_sfc_num'].astype(str).values
    ...
    ax.set_title('E15 Experiment: DHEFT vs NHEFT Makespan', fontsize=14, fontweight='bold')
    ax.set_xticks(x_pos)
    ...
    plt.tight_layout()
    bar_path = os.path.join(result_dir, 'e15_makespan_comparison.png')
