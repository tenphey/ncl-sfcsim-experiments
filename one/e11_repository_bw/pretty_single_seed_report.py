...
Examples:
  python3 pretty_single_seed_report.py debug_1405_100
  python3 pretty_single_seed_report.py debug_1405_100 ""
  python3 pretty_single_seed_report.py debug_1405_100 dheft
  python3 pretty_single_seed_report.py debug_1405_100 nheft
"""
...
DHEFT_START_RE = re.compile(
    r"^\[DHEFT-START\]\s+Scheduling\s+VNF=(?P<vnf>\d+),.*?candidateVCPU=(?P<count>\d+)"
)
DHEFT_CAND_RE = re.compile(
    r"^\[DHEFT-CAND\]\s+VNF=(?P<vnf>\d+),.*?->\s+vCPU=(?P<vcpu>[^,]+).*?,\s+est=(?P<est>[-\d.]+)(?:,\s+dlTime=(?P<dl>[-\d.]+))?,\s+execTime=(?P<exec>[-\d.]+),\s+finish=(?P<finish>[-\d.]+)"
)
DHEFT_SELECT_RE = re.compile(
    r"^\[DHEFT-SELECT\]\s+VNF=(?P<vnf>\d+),.*?->\s+vCPU=(?P<vcpu>[^,]+).*?,\s+start=(?P<start>[-\d.]+),\s+finish=(?P<finish>[-\d.]+)"
)
...
NHEFT_START_RE = re.compile(
    r"^\[NHEFT-START\]\s+Scheduling\s+VNF=(?P<vnf>\d+),.*?candidateVCPU=(?P<count>\d+)"
)
NHEFT_CAND_RE = re.compile(
    r"^\[NHEFT-CAND\]\s+VNF=(?P<vnf>\d+),.*?->\s+vCPU=(?P<vcpu>[^,]+).*?,\s+est=(?P<est>[-\d.]+)(?:,\s+dlTime=(?P<dl>[-\d.]+))?,\s+execTime=(?P<exec>[-\d.]+),\s+finish=(?P<finish>[-\d.]+)"
)
NHEFT_SELECT_RE = re.compile(
    r"^\[NHEFT-SELECT\]\s+VNF=(?P<vnf>\d+),.*?->\s+vCPU=(?P<vcpu>[^,]+).*?,\s+start=(?P<start>[-\d.]+),\s+finish=(?P<finish>[-\d.]+)"
)
...
def parse_dheft_nheft(log_path: Path):
    ...
    return data, makespans, vnf_order, lines
...
def render_dheft_nheft_merged(report_title: str, log_name: str, lines, makespans, data, vnf_order):
...
def main():
    if len(sys.argv) < 2:
        ...
        print("示例: python3 pretty_single_seed_report.py debug_1405_100")
        print('示例: python3 pretty_single_seed_report.py debug_1405_100 ""')
        print("示例: python3 pretty_single_seed_report.py debug_1405_100 dheft")
        print("示例: python3 pretty_single_seed_report.py debug_1405_100 nheft")
        sys.exit(1)
    ...
    if is_merged:
        ...
        data, makespans, vnf_order, lines = parse_dheft_nheft(log_path)
        report = render_dheft_nheft_merged(
            "DHEFT / NHEFT 单种子保序简化报告",
            log_path.name,
            lines,
            makespans,
            data,
