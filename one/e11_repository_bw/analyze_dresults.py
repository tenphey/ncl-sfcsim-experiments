...
Usage:
  python3 analyze_dresults.py <result_folder>
  python3 analyze_dresults.py run_20260526_120000
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: analyze_dresults.py <result_folder>")
        raise SystemExit(1)
    ...
    # Prefer HEFTD column. Keep backward compatibility with legacy CSVs that only have HEFT.
    if "HEFTD" not in df.columns:
        if "HEFT" in df.columns:
            print("Warning: HEFTD column not found in CSV, fallback to HEFT column as HEFTD.")
            df["HEFTD"] = df["HEFT"]
        else:
            raise SystemExit("Missing required column: HEFTD (or legacy HEFT)")

    df = df[(df["HEFTD"] > 0) & (df["DHEFT"] > 0) & (df["NHEFT"] > 0)]

    df["gain_N_over_D"] = (df["DHEFT"] - df["NHEFT"]) / df["DHEFT"] * 100
    df["gain_D_over_HD"] = (df["HEFTD"] - df["DHEFT"]) / df["HEFTD"] * 100
    ...
    for rb, group in grouped:
        n_runs = len(group)
        heftd_mean = group["HEFTD"].mean()
        heftd_std = group["HEFTD"].std()
    ...
    # Plot 2b: Grouped bar chart - HEFTD vs DHEFT vs NHEFT
    fig, ax = plt.subplots(figsize=(12, 7))

    heftd_means = summary_df["HEFTD_mean"].values
    x_pos = np.arange(len(bws))
    bar_width = 0.26
    ...
    ax.set_ylabel("Average Makespan (seconds)", fontsize=13, fontweight="bold")
    ax.set_title(
        "E11 Experiment: HEFTD vs DHEFT vs NHEFT Makespan by Repository Bandwidth",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xticks(x_pos)
    ...
    plt.close()
    print(f"Grouped bar chart (HEFTD vs DHEFT vs NHEFT) saved: {chart_path_bar_all}")
